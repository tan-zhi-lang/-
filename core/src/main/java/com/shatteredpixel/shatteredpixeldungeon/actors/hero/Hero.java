...
public class Hero extends Char {
    ...
    public String 海克斯描述(String 选择){
        switch(选择){
            ...
            case "安内":return "对1格内的敌人造成攻击+75%伤害，对之外的敌人仅67%伤害";
...
//        回百分比血(0.05f);
    ...
    public float 增加防御(){
        ...
        x-=虚无透纱.减少();
        if(符文("魔抗转防御"))
        x+=1/RingOfElements.resist(this,Dungeon.class);
...
//        if (belongings.weapon1() != null) {
//            return belongings.weapon1().canReach(this, enemy.pos);
//        }
//        if (belongings.weapon2() != null) {
//            return belongings.weapon2().canReach(this, enemy.pos);
//        }
...
//        return 攻击范围(enemy);
    ...
    public float 攻击延迟() {
        ...
        if(!空手()) {
            ...
            if(belongings.weapon1()!=null){
                delay=belongings.weapon1().augment.delayFactor(delay);
            }
            if(belongings.weapon2()!=null){
                delay=belongings.weapon2().augment.delayFactor(delay);
            }
        }
    ...
    public boolean 免疫(Class effect) {
        ...
        if (heroClass(HeroClass.机器)||种族天赋.equals("机器")) {
            immunes.addAll(Property.机械.resistances());
            immunes.addAll(Property.电.resistances());
        }
        if (heroClass(HeroClass.灵猫)||heroClass(HeroClass.鼠弟)) {
            immunes.addAll(Property.动物.resistances());
        }
                                ...
                                protected void onComplete() {
                                    ...
                                    for (int i : PathFinder.相邻) {
                                        if (Dungeon.level.map[action.dst + i] == Terrain.MINE_CRYSTAL) {
                                            ...
                                            if(Dungeon.branch==1){
                                                if(算法.概率学(1/4f))
                                                    Level.set(action.dst+i,Terrain.EMPTY_DECO);
                                                else
                                                    Level.set(action.dst+i,Terrain.EMPTY);
                                            }
                                            else{
                                                if(算法.概率学(1/8f))
                                                    Level.set(action.dst+i,Terrain.CHASM);
                                                else if(算法.概率学(1/4f))
                                                    Level.set(action.dst+i,Terrain.EMPTY_DECO);
                                                else
                                                    Level.set(action.dst+i,Terrain.EMPTY);
...
//        GameScene.flashForDocument(Document.ADVENTURERS_GUIDE, Document.GUIDE_INTRO);
    ...
    private float 攻击特效(Char enemy,float damage){
        ...
        if (enemy!=null&&subClass(HeroSubClass.狙击手)&&职业精通()
            &&!(belongings.投掷武器() instanceof 灵能短弓.SpiritArrow)) {
            Actor.add(new Actor() {
...
//        }
            ...
            for (int n : PathFinder.相邻){
                ...
                if(c!=null&&c.alignment == Alignment.ENEMY&&Dungeon.level.heroFOV[c.pos]){
                    ...
                    sprite.attack(enemy.pos,new Callback() {
                        ...
                        public void call() {
                            AttackIndicator.target(enemy);
                            if (attack(enemy,天赋点数(Talent.灵动尾击,0.4f),heroDamage(武力之戒.heromin(),武力之戒.heromax()),1)) {
                                Sample.INSTANCE.play(Assets.Sounds.HIT);
                            }
                            Invisibility.notimedispel();
                            spendAndNext(0);
    ...
    private float 物理抗性(Char enemy,float damage){
        ...
        if(enemy!=null&&视野范围(enemy))
            damage*=1-天赋点数(Talent.戒备,0.2f/3f);
        if(天赋(Talent.欧皇庇护))
...
//                else
    ...
    public boolean handle(int cell) {
        ...
        } else if ((Dungeon.区域()==3||Dungeon.level instanceof MiningLevel)&&
        ...
        } else if (!Dungeon.bossLevel()&&belongings.getItem(钻石镐.class)!=null&&
                   (Dungeon.level.map[cell] == Terrain.WALL
                    || Dungeon.level.map[cell] == Terrain.WALL_DECO
                    || Dungeon.level.map[cell] == Terrain.MINE_CRYSTAL
                    || Dungeon.level.map[cell] == Terrain.MINE_BOULDER)) {
...
//                        Dungeon.depth < 26 || //26+不能上楼
    ...
    public void 经验(int exp,Class source) {
        ...
        if (sprite!=null&&exp > 0) {
           sprite.showStatusWithIcon(CharSprite.增强绿,Integer.toString(exp),FloatingText.EXPERIENCE);
        }
...
//                Buff.施加(this, Hunger.class).吃饭(-4);
    ...
    @Override
    public float 法术穿透() {
        float 法术穿透 = 1-RingOfElements.resist(this);
        ...
        if(职业精通){
            switch(sc){
                ...
                case 神偷无影:
                    ...
                    break;
                case 狙击手:
                    s+= "除灵能短弓外的攻击会施加狙击标记，灵能短弓对狙击标记敌人攻击取决于不同的强化方式。";
                    break;
                case 守望者:
