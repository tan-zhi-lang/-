...
import com.shatteredpixel.shatteredpixeldungeon.actors.hero.spells.木遁;
import com.shatteredpixel.shatteredpixeldungeon.actors.hero.spells.死血;
import com.shatteredpixel.shatteredpixeldungeon.actors.hero.spells.火墙术;
...
import com.shatteredpixel.shatteredpixeldungeon.effects.Swap;
...
import com.shatteredpixel.shatteredpixeldungeon.items.food.蛋糕;
...
import com.shatteredpixel.shatteredpixeldungeon.items.用品.贤者之石;
import com.shatteredpixel.shatteredpixeldungeon.items.用品.超级升级符石;
...
public class Hero extends Char {
    ...
    public void put1(String s,int 权重,int 获得){
        海克斯表.put(s,100+权重*10+获得);
    }
    public void put1(String s,int 权重){
        海克斯表.put(s,100+权重*10+0);
    }
    public void put2(String s,int 权重,int 获得){
        海克斯表.put(s,200+权重*10+获得);
    }
    public void put2(String s,int 权重){
        海克斯表.put(s,200+权重*10+0);
    }
    public void put3(String s,int 权重,int 获得){
        海克斯表.put(s,300+权重*10+获得);
    }
    public void put3(String s,int 权重){
        海克斯表.put(s,300+权重*10+0);
    }
    ...
    public void 海克斯重置(){
        ...
        //33333333333333
        {
            ...
            put3("牙仙子",2);
            put3("真灵魂虹吸",2);
            put3("591铂金60金15银3铜",2);
            put3("风雷翅:左风右雷随心意而动",2);
            put3("5号化合剂",2);
            put3("万魂幡力",2);
            put3("海克斯获取:屠杀",2);
            put3("智慧末刃:医学奇迹",2);
    ...
    public void 分配海克斯(){
        ...
        if(belongings.hasItem(灵能短弓.class))
            权重3海克斯("升级灵能短弓");

        if(belongings.hasItem(狂妄皇冠.class))
            权重3海克斯("升级狂妄皇冠");

        if(belongings.hasItem(荆棘斗篷.class))
            权重3海克斯("升级荆棘斗篷");
        ...
        if(belongings.hasItem(生命蜡烛.class))
            权重3海克斯("升级生命蜡烛");

        if(belongings.hasItem(灵魂焰灯.class))
            权重3海克斯("升级灵魂焰灯");

        if(belongings.hasItem(猩红散华.class))
            权重3海克斯("升级猩红散华");

        if(belongings.hasItem(无影剑.class))
            权重3海克斯("升级无影剑");

        if(belongings.hasItem(变态刀.class))
            权重3海克斯("升级变态刀");

        if(belongings.hasItem(无尽之刃.class))
            权重3海克斯("升级无尽之刃");

        if(belongings.hasItem(饮血之剑.class))
            权重3海克斯("升级饮血之剑");

        if(belongings.hasItem(破败王剑.class))
            权重3海克斯("升级破败王剑");

        if(belongings.hasItem(水袋.class))
            权重3海克斯("升级水袋");
        if(belongings.armor1()!=null&&belongings.armor1().专属)
            权重3海克斯("随行圣衣");
        if(belongings.hasItem(九龙拉管.class))
            权重3海克斯("极限剂量");
        if(belongings.hasItem(Bomb.class))
            权重3海克斯("王者之翼手雷");
        if(belongings.hasItem(LiquidMetal.class)){
            权重3海克斯("金验");
            权重3海克斯("吃金液");
            权重3海克斯("圣水");
            权重3海克斯("要射金液了");
        }
        if(belongings.hasItem(石头.class)){
            权重3海克斯("升级石头");
            权重3海克斯("冰雹");
            权重3海克斯("圣石");
            权重3海克斯("碎石");
        ...
        if(belongings.hasItem(WandOfMagicMissile.class)||
           belongings.hasItem(WandOfBlastWave.class)
        ){
            权重3海克斯("属性:空");
        }
        if(belongings.hasItem(WandOfCorruption.class)||
           belongings.hasItem(WandOfTransfusion.class)
        ){
            权重3海克斯("属性:虚");
        }
        if(belongings.hasItem(焰浪法杖.class)||
           belongings.hasItem(烈焰法杖.class)
        ){
            权重3海克斯("属性:火");
        }
        if(belongings.hasItem(冰海法杖.class)||
           belongings.hasItem(WandOfFrost.class)
        ){
            权重3海克斯("属性:冰");
        }

        if(belongings.hasItem(WandOfPrismaticLight.class)||
           belongings.hasItem(棱镜法杖.class)
        ){
            权重3海克斯("属性:光");
        }
        if(belongings.hasItem(WandOfDisintegration.class)||
           belongings.hasItem(影织法杖.class)
        ){
            权重3海克斯("属性:暗");
        }
        if(belongings.hasItem(WandOfLivingEarth.class)||
           belongings.hasItem(落石法杖.class)
        ){
            权重3海克斯("属性:地");
        }
        if(belongings.hasItem(心之钢.class)){
            权重3海克斯("钢化你心");
        }
        if(belongings.hasItem(铜钱剑.class)){
            权重3海克斯("道长");
        }
        if(空手()){
            权重3海克斯("认真的一拳");
        ...
        if(
                belongings.hasItem(火焰剑.class)||
                belongings.hasItem(地裂镰.class)||
                belongings.hasItem(寒冰镖.class)||
                belongings.hasItem(闪电双截棍.class)
           ){
            权重3海克斯("黄金忍者");
        }
        if(
        ...
        if(Challenges.activeChallenges()>3&&
           炼狱设置.使用()>2&&
           赛季设置.使用()>2&&
    ...
    public String 海克斯描述(String 选择){
        switch(选择){
            ...
            case "吞噬星空":return "暗金有吞噬按钮，吞噬之后消耗掉暗金并0.65%综合属性";
            case "石头剪刀布":return "石头的攻击33%概率造成9倍伤害";
            case "属性叠":return "获得2个属性锻造器";
            case "畸变选择":return "海克斯秘卷的选择按顺序123级3个选择";
            case "好记性如烂笔头":return "之后获得的海克斯不会再备注，但是海克斯秘卷刷新次数+4";
            case "神赐":return "升级所需经验x3，祝福时同时x3综合属性";
            case "逃脱者":return "与视野内敌人距离非相邻时候移速+35%";
            case "铁男单挑竞技场":return "决斗秘卷中击杀敌人会永久获得敌人10%最大生命、最大攻击、最大防御、攻速、移速、命中、闪避";
            case "真穿甲流":return "穿甲+最大护甲，最大防御+穿甲";
            case "升级升级:力量":return "+50%等级力量";
            case "升级升级:魔力":return "+50%等级魔力";
            case "贯穿星辰":return "攻击会无限穿过敌人对后面的所有敌人也物理伤害，直到碰到墙体";
            case "第一滴血":return "蛇皮走位满层使攻击伤害+45%";
            case "艾哲红石":return "背包中有天狗面具时，元素抗性+90%";
            case "吃金液":return "金液可以吃，填充450饥饿";
            case "初生之誓":return "暴击额外造成1%魔力x攻击伤害的魔法伤害";
            case "这是不是太隐晦了":return "金液可以转换为2瓶隐形药剂";
            case "春暖花开":return "每天地图生长新的草丛、植物";
            case "只要1级海克斯":return "只能刷到1级海克斯";
            case "进攻是最好的防御":return "最大防御+最大攻击，最小防御+最小攻击，防御+攻击";
            case "金验":return "金液新增按钮，可以转变为经验药剂";
            case "圣水":return "1个金液可以填充4/20水袋";
            case "算你倒霉":return "幸运值-1";
            case "爆伤":return "攻击暴击会触发炸弹爆炸(不过不会和炸弹一样炸弹英雄以及破坏物品，且伤害减半)";
            case "鹰头猫":return "+4%x视野范围的暴击率";
            case "溜冰":return "你的移动在1格时会多走一格，敌人攻速和移速仅33%";
            case "脆皮秒了":return "首次攻击秒杀了敌人则恢复15%最大生命、最大护甲、饥饿";
            case "升级升级卷轴:武器专精":return "升级卷轴无法提升法杖等级，但是对武器额外升1级";
            case "升级升级卷轴:法杖专精":return "升级卷轴无法提升武器等级，但是对法杖额外升3级";
            case "占位符":return "无效果";
            case "王炸":return "狂妄皇冠扔出可以产生炸弹效果";
            case "寸止":return "任何爆炸效果每次爆炸都会使其爆炸效果永久+30%";
            case "毁灭高潮":return "产生任何爆炸效果获得15金液";
            case "紫极魔瞳":return "视野范围+35%魔力";
            case "湿了":return "每回合将你的所在位置变成水";
            case "火把之神的馈赠":return "光照范围+4";
            case "燃血法":return "攻击伤害+10%生命";
            case "动力法术学":return "半血以下时，魔力+20%";
            case "免疫学":return "50%概率免疫负面Buff";
            case "附魔神剑":return "剑武器攻击会附带背包所有武器的附魔";
            case "白骨皮肉":return "所有敌人视为恶魔、亡灵";
            case "橡皮长枪手":return "搜索范围+2倍攻击范围";
            case "迅雷不及掩耳":return "暴击率+1%敏捷";
            case "炼狱绳":return "自残绳使用伤害降低至15%最大生命的魔法伤害，并永久+0.5力量";
            case "罪恶快感":return "击杀敌人获得5回合30%攻速和50%移速";
            case "击杀获得护盾":return "击杀会获得15%最大攻击的护盾";
            case "043809438":return "任何英雄都判定为女人，并获得妇女节加成";
            case "雌雄同体":return "具备男英雄和女英雄的所有特征";
            case "食以民为天":return "击杀敌人恢复敌人最大生命的饥饿";
            case "拿来吧你":return "首次攻击获得敌人战利品";
            case "星星之火可以燎原":return "燃烧伤害会x剩余燃烧回合";
            ...
            case "怕光":return "对视野范围内的敌人攻击伤害+10%，游戏亮度低，且网格可视度关闭";
            ...
            case "幸运":return "幸运值+1";
            case "来互秒":return "敌人最大生命仅20%，但是攻击x3";
            case "小源魔力":return "最大生命-15%，魔力+最大生命9%";
            case "不耐揍的敌人":return "敌人最大生命值仅90%";
            ...
            case "死神":return "对残血敌人+30%攻击伤害";
            case "交锋":return "攻击对满血敌人必定暴击";
            case "子时已到":return "23:00-1:00，综合属性+500%";
            case "巫师式思考":return "魔力+15";
            case "仿生:巨钳螃蟹":return "必定闪避被发现回合小于5的敌人";
            case "狂热者":return "+3%魔力的暴击率和攻速";
            case "磨刀霍霍向猪羊":return "每次交互魔法绵羊永久+刀类武器7.5%攻击";
            case "冰寒":return "敌人的额外移速-0.1(固定减移速)";
            case "吃虫饱兰博":return "击杀水蛭，永久+20最大生命";
            case "晶体护甲":return "暴击恢复45%最大护甲";
            case "侧滑":return "闪避恢复3%最大生命";
            case "急急小子":return "移速受武器，法杖和神器的75%充能影响";
            case "你已经是一名成熟的英雄了":return "升级时会恢复所有生命、护甲、饥饿";
            case "双剑华斩":return "副武器攻击效果+60%";
            case "胡来的左手":return "副武器攻击效果-30%~+45%";
            case "泰君":return "综合属性+2%x泰拉瑞亚物品";
            ...
            case "双重打击":return "每第4次攻击会额外攻击一次，并+50%伤害";
            case "大赦天下":return "在2区和4区，刷怪数量x2，刷怪速度x2，但是掉率x10";
            case "暴雨":return "在水中，每回合叠加1层，每层提供5%综合属性，离开水会移除所有层";
            case "我让你额啊":return "受到致命伤害不会有声音和打断英雄，并且恢复15%最大护甲";
            case "三重暴击":return "33%暴击率在暴击时额外x2次暴击伤害";
            case "明牌":return "感知范围+15";
            case "三国杀:黄盖":return "每累计受到25%最大生命的伤害都会随机获得一个物品";
            case "三国杀:华佗":return "饮用其他药剂33%概率额外附带治疗药剂效果";
            case "严厉斥责":return "暴击伤害随机+0~暴击率%";
            case "专横":return "力量+15%最大生命";
            case "神速力":return "敏捷+15";
            case "虚空灌注":return "魔力+7.5%最大生命";
            case "赐死剑气":return "攻击额外造成10%敌人最大生命值的魔法伤害，并恢复额外伤害的生命值，冷却20回合，攻击会减少2回合冷却，暴击额外减少2";
            case "吸元秘术":return "攻击恢复10%最大生命值，但是冷却30回合，攻击和防御都会减少三回合冷却，如果是对老鬼则额外减少1.5回合冷却";
            case "绝对爆率":return "掉率必定100%";
            case "行万里路":return "总移速仅80%，但是综合属性+25%";
            case "面壁者":return "相邻每有一个墙体提供8%元素抗性";
            case "鬼脑":return "击杀最大攻击大于你的敌人，永久+敌人最大攻击的最大攻击";
            case "杰作锻造锤":return "锻造锤升级限制+14";
            case "作弊:我能回城":return "阅读来去秘卷获得1张来去秘卷";
            case "升级石头":return "石头+4阶级";
            case "冰雹":return "石头首次攻击冻结敌人10回合";
            case "碎石":return "石头攻击会施加100%伤害的流血";
            case "伶俐斗士":return "受到的伤害减少百分之0.2%魔力";
            case "坚定风采":return "防御和魔抗+20%，残血时翻倍";
            case "暗裔恶魔":return "对老鬼和小老鬼+15%全能吸血";
            ...
            case "给你打成折叠屏":return "首次攻击永久将敌人高度变为一个面，并使其攻击伤害-20%";
            case "凯旋":return "击杀获得33%金币并恢复10%已损失生命";
            case "坚固":return "最大防御和魔抗+15";
            case "手慢无":return "购买商店物品33%概率不消耗金币";
            case "坚韧之躯":return "根据已损失生命，最多20%物理抗性";
            case "独眼":return "左边的视野你看不见，但是视野范围x2，感知范围+5";
            case "随行圣衣":return "英雄初始防具+4阶级";
            case "升级水:纯水":return "在水中时，每回合10%概率生成露珠";
            case "瞄准脑袋":return "暴击率最高50%，且超过50%的暴击率同超过100%暴击率一样转暴击伤害";
            case "贪婪之握":return "全能吸血+1%，击杀区域的BOSS+1%全能吸血(1~4区域)";
            case "心灵净化":return "击杀敌人会对周围敌人造成25%生命的魔法伤害";
            case "给我暴！":return "暴击伤害+85%";
            case "农夫三拳":return "每第三次攻击会额外攻击3次";
            case "原神:不屈":return "非健康血量时，最小命中为最大命中，暴击伤害+50%";
            case "盈能攻击！":return "蛇皮走位满层额外造成敌人20%生命的魔法伤害";
            case "无畏北伐":return "每次脱离战斗状态恢复5%最大生命";
            case "黑洞":return "敌人相邻深渊都会每回合受到敌人5%最大生命值魔法伤害";
            case "砸开那颗蛋":return "你的护盾在减少时会对周围敌人造成200%减少值的魔法伤害";
            case "宇宙安全声明":return "最大攻击+80/视野范围";
            case "偷偷变强一鸣惊人":return "获得隐形时，力量+1";
            case "烈士墓":return "敌人相邻有坟墓时，会每回合受到敌人30%最大生命值的魔法伤害";
            case "神射法师":return "攻击伤害+75%魔力";
            ...
            case "自然即是治愈":return "在草丛中每回合恢复5%最大生命";
            case "生命渗透压":return "吸血+0.07%最大生命值";
            ...
            case "三国杀:司马懿":return "受到伤害如果伤害来源是敌人则获得其战利品，如果有此敌人的战利品则受到的伤害为0";
            case "幸不辱命":return "最大生命+幸运值x100";
            case "苦行僧":return "总移速-15%，受伤获得25回合净化";
            case "奉献":return "防御会对首次攻击你的敌人造成30%生命的魔法伤害";
            case "核聚变":return "每有爆炸效果产生，都会使你恢复所有生命值";
            case "风暴":return "在攻击触发额外攻击次数时，攻速翻倍，攻击翻倍";
            case "致盲":return "失明会使敌人命中为0，且首次攻击对敌人施加5回合失明";
            case "伤敌100自损100":return "自残绳会对所有敌人造成伤害";
            case "真大力之泉":return "如果是英雄力量+175，如果是武器、法杖、戒指、防具+35级";
            case "高倍望远镜":return "攻击根据距离，最多在4格+40%伤害";
            case "玉足":return "女人裸衣总移速-10%，但是防御+800%";
            case "我让你闪避":return "取消敌人的闪避机制";
            case "我让你跑":return "敌人移速大于你时，每回合受到你最大攻击的魔法伤害";
            case "夜狩":return "击杀敌人获得20回合隐形";
            case "饿死鬼投胎":return "饥饿为0时吃饭会恢复20%最大生命";
            case "我让你破坏":return "物品不会被烧毁，炸毁，冻碎";
            case "极限剂量":return "九龙拉管每次使用永久+10%已损失生命的最大生命值";
            case "一口吃得成大胖子":return "吃饭+5最大生命和0.025大小";
            case "真:吸血鬼":return "吸血+全能吸血大于20%，则治疗护盾+250%";
            case "任务:海克斯获取":return "即刻:获得1个刷新次数为0海克斯秘卷\n需求:以非升级方式获得1个海克斯秘卷\n奖励:获得2个海克斯秘卷";
            case "大胃王":return "饥饿速度仅1/3，再生速度x3，饱腹回合上限+900";
            case "右手麒麟臂":return "攻击+15%，但是副武器攻击效果-15%";
            case "备用治疗药剂":return "饮用治疗50%获得1个治疗药剂";
            case "备用口粮":return "吃下食物有50%概率获得食物";
            case "俺是一个大信球":return "当幸运值是负数时，综合属性-幸运值倍";
            case "wc冰!":return "冻结伤害+最小攻击+力量+最大攻击";
            case "2233暴击":return "暴击时50%暴击率概率额外造成50%暴击伤害，之后33%暴击率的概率额外造成50%暴击伤害";
            case "幻影突袭":return "闪避后攻击必定暴击";
            case "燕返":return "攻击效果+1";
            case "血腥洞察":return "受到超过4%最大生命的伤害后攻击必定暴击";
            case "升级综合属性":return "综合属性现在会改变治疗护盾，为150%效果";
            case "缩小射线":return "战斗状态的敌人大小-0.15，且敌人的攻击伤害为大小";
            case "我是瘦子":return "你不能吃饭，只能在饥饿值为0的时候才能吃饭，且饥饿速度仅25%，但是饥饿的伤害翻倍";
            case "面包和黄油":return "法杖充能速度+50%";
            case "面包和奶酪":return "神器充能神器+50%";
            case "面包和果酱":return "武器充能速度+50%";
            case "卷轴赌狗":return "随机45张卷轴";
            case "药剂赌狗":return "随机45瓶药剂";
            case "物品赌狗":return "随机60个物品";
            case "起源:剑":return "剑类武器等级+20%魔力";
            case "鬼刀一开看不见":return "使用武技后总移速x3，持续5回合";
            case "你好我能换个位置吗":return "攻击与敌人交换位置";
            case "牙结石":return "吃饭时恢复7.5%的饭量的护甲";
            case "先攻":return "首次攻击伤害x1.1，并获得伤害的金币";
            ...
            case "什么档次也配和我同等级":return "对同等级敌人攻击伤害+100%";
            case "屠夫":return "攻击生物类型敌人时候1/8概率掉落生肉";
            case "升级死神镰刀":return "无需残血也能造成暴击伤害，且残血也能再次x暴击伤害";
            case "电能震荡":return "冲击波法杖额外造成最大攻击力的伤害";
            case "升级破碎像素地牢":return "地牢敌人数量和刷怪速度x3，但是掉率几率x4";
            case "专注":return "攻击前3回合也攻击过的敌人时，攻击伤害+15%";
            case "鬼影迷踪":return "移速+1.5%魔力";
            case "力速双A替身":return "替身+45%总攻速和总攻击";
            case "收集者":return "生命值低于5%的敌人会被秒杀，并掉落金币";
            case "骰子收集者":return "海克斯秘卷刷新次数+2";
            case "升级全能吸血":return "全能吸血+2倍吸血，但是吸血无效";
            case "小怪猎手":return "对非BOSS+15%攻击伤害";
            case "猛攻":return "吸血和全能吸血无效，但攻击伤害+50%吸血+50%全能吸血";
            case "风语者的祝福":return "防御+150%治疗护盾";
            case "远视人启动":return "视野范围+5";
            case "乌龟": return "总移速-10%，防御+50%";
            case "失败的Man":return "力量+40%";
            case "止戈":return "攻击伤害+20%，-10%物理抗性";
            ...
            case "升级荆棘斗篷":return "最大防御+最大护甲x(1+神器等级)/3，充能速度+45%";
            case "走完红色路线":return "中国国旗不再减速，+20%物理抗性";
            case "升级蓄血圣杯":return "生命再生额外恢复2.25%已损失生命";
            case "升级丰饶之角":return "每层充能填充+100%，充能速度+45%";
            case "升级法师魔杖":return "+50%魔杖等级";
            case "铁布衫":return "布甲防御+550%";
            ...
            case "保底机制":return "随机到保底海克斯的概率x3";
            case "刃下生风":return "移速+10%穿甲和法穿、100%护甲穿透和法术穿透";
            case "无法上吊之物":return "自残绳子使用不会受到伤害，并且恢复2%最大生命";
            case "升级狂妄皇冠":return "+20穿甲，狂妄在连杀时回血20%已损失生命，并且连杀状态+25%移速";
            case "神圣干预":return "每25回合，获得3回合无敌，冷却从神器充能收益";
            case "穿针引线":return "+20%护甲穿透，+20%法术穿透";
            case "起源:切断与结合":return "+40%护甲穿透";
            case "王者之翼手雷":return "炸弹伤害x4";
            case "回力OK标":return "每10回合对视野内一个敌人扔出一个回旋镖，对敌人造成50%最大攻击力的魔法伤害，冷却从神器充能收益";
            case "升级灵能短弓":return "射击伤害+350%";
            case "急救用具":return "治疗护盾+25%";
            case "快手":return "装备和卸下装备不消耗回合";
            case "炸死这群王八蛋":return "获得25个炸弹";
            case "蜂后":return "获得25个蜂蜜罐";
            case "属性:空":return "魔弹法杖和冲击波法杖魔力收益+50%";
            case "属性:虚":return "腐化法杖和注魂法杖魔力收益+50%";
            case "属性:火":return "焰浪法杖和烈焰法杖魔力收益+50%";
            case "属性:冰":return "冰海法杖和冰霜法杖魔力收益+50%";
            case "属性:光":return "棱光法杖和棱镜法杖魔力收益+50%";
            case "属性:暗":return "解离法杖和影织法杖魔力收益+50%";
            case "属性:地":return "灵壤法杖和落石法杖魔力收益+50%";
            ...
            case "溢流":return "法杖消耗x2，但是魔力收益x2";
            case "尊我为王":return "在获得Yendor护符时，会随机获得一个3级海克斯";
            case "适应性头盔":return "受到与上次受到同类型伤害时-20%伤害";
            case "民以食为天":return "获得20份随机食物";
            case "魔术基盘":return "触发复活，魔力永久+5";
            case "投影魔术":return "总魔力-10%，所有武器都具备回旋镖的特性";
            case "一板一眼":return "攻速固定1，攻速+25%，攻速变为提升攻击";
            case "暴风吸入":return "吸血+10%暴击伤害";
            ...
            case "升级灵魂焰灯":return "击杀判定和击杀获取x2";
            case "升级生命蜡烛":return "无敌回合x2，受到伤害/2";
            case "升级自然之履":return "踩草的自然层+2，自然层=神器等级+1，种子掉率=1/(25f - 自然层 *4f)，露珠掉率=1/(6f - 自然层 /2f)";
            case "会心治疗":return "+25%暴击率，治疗护盾可以暴击然后额外恢复暴击伤害的量";
            case "会心防御":return "+25%暴击率，根据暴击率最多+20%物理抗性";
            case "会心闪避":return "暴击率+25%，闪避可以暴击额外+暴击伤害";
            case "穿甲的宠爱":return "穿甲+15%最大防御";
            case "左拳伤害高右拳高伤害":return "每第二次攻击伤害和攻速+15%";
            ...
            case "虚幻武器":return "对目标施法同时会施加100%伤害的攻击";
            case "实用海克斯移除器":return "获得4个海克斯移除器";
            case "海克斯秘卷移除器":return "海克斯秘卷新增移除，与海克斯移除器一样的效果，不过使用会消耗海克斯刷新次数，清零则消失";
            case "纪元":return "攻击伤害+敌人被发现的回合x5";
            ...
            case "捐赠":return "金币+1000，能量+125";
            case "佯攻":return "首次攻击伤害仅0%，之后的攻击伤害+15%";
            case "唯快不破":return "攻击根据移速差距增伤，每比敌人多3%+1%伤害";
            case "残暴之力":return "最大攻击+16，穿甲+8";
            case "海克斯获取:幸运":return "每过去一天时，50%概率获得海克斯秘卷";
            case "海克斯获取:收获":return "踩踏草丛，1/300获得海克斯秘卷";
            case "裁决使":return "对残血敌人+10%伤害，同时在击杀会恢复武器法杖神器的能量";
            ...
            case "来吧老弟":return "攻击伤害+20%，敌人攻击伤害+8%";
            case "永不动摇":return "静止状态超过5回合时，每回合恢复5%已损失生命";
            case "太有用了":return "随机出高等级的海克斯概率x4";
            case "5阶武器库":return "随机+5等级的三选一5阶武器";
            case "法杖库":return "随机+5等级的三选一法杖";
            case "戒指库":return "随机+5等级的三选一戒指";
            case "凡灵双剑":return "攻击伤害25%转换为魔法伤害";
            case "黑幕":return "属性面板不显示，综合属性+30%";
            case "天狗的宠爱":return "发现敌人会投掷一个魔法箭矢，造成50%最大攻击的魔法伤害";
            case "全能石":return "获得10张蜕变秘卷";
            case "铁板鞋":return "最大防御+25，+10%物理抗性，移速+30%";
            case "雷霆大A":return "攻击伤害+25%";
            case "大墟":return "在装饰墙和出入口相邻时，综合属性+65%，光照范围+8";
            case "狂战之怒":return "根据已损失生命最多+100%攻速和10%吸血";
            case "雷霆腰带":return "攻击永久+0.075最大防御和最大魔抗";
            case "要射金液了":return "攻击超过3距离会获得1个金液";
            case "你才是老鬼":return "最大生命x10，但是敌人+25%吸血，刷怪数量x2，刷怪速度x10，敌人移速x3，敌人攻击额外造成你1%最大生命的魔法伤害";
            case "主角模板":return "天赋点数+8";
            case "术士果汁盒":return "全能吸血+0.7%魔力";
            case "好运来":return "+25%暴击率，+2幸运值";
            case "无限变强":return "每次复活获得一个随机海克斯";
            case "倘若我说我绷住了呢":return "20%概率无视所受伤害";
            case "追上命都给你噢":return "攻击x移速";
            case "给我砸":return "所有武器+15%麻痹";
            case "死亡复活":return "击杀敌人触发一次你的复活";
            case "心之疗":return "首次攻击+0.3%治疗护盾";
            case "仆从大师":return "友军攻击伤害+40%，所受伤害-30%，大小+0.4";
            ...
            case "追雷赶电靴":return "移动对相邻敌人造成移动敏捷x距离x移速的魔法伤害";
            case "综合公式":return "综合属性x(((2+3.14)x6.18-0.7652)/10)-1.1)";
            case "未知防御":return "物理抗性+幸运值x10";
            case "光合作用":return "地图中的每个草丛和植物都会为你提供0.03再生";
            case "升级升级:幸运":return "+15%等级幸运";
            case "哎哟我的硬币":return "击杀敌人会在相邻格子生成15金币";
            case "得劲儿魔法":return "魔力+地牢层数x1.5";
            case "高压锅":return "每回合对相邻内敌人造成1%最大生命的无限叠加伤害";
            case "猩红之力":return "每块生肉为你提供0.3%全能吸血";
            case "法态:穿透":return "+100%元素抗性，在造成魔法伤害时，会-100%元素抗性持续5回合";
            case "猩红诅咒":return "移除生命再生，根据已损失生命最多+200%治疗护盾";
            case "升级升级:暴击":return "+3%等级暴击率";
            case "溶烛化紫":return "击杀敌人永久+1.5%攻速";
            case "英雄平衡性增益":return "根据敌人生命攻击伤害最多+40%，治疗护盾+20%";
            case "狂怒攻击":return "+25%暴击率，你的攻击无法暴击，但是攻击伤害x(1+暴击率x暴击伤害)";
            case "你只是失去了生命":return "击杀敌人66%概率不触发击杀效果，33%概率触发x3次";
            case "过敏性鼻炎":return "每回合2%概率地震，同时永久+0.02力量";
            case "这不合理":return "老鬼和小老鬼的生命，攻击，防御，命中，闪避，攻速，移速仅20%，但是其他怪物为200%";
            case "太合理了":return "老鬼和小老鬼，生命，攻击，防御，命中，闪避，攻速，移速为300%，其他怪物为30%";
            case "这太几把黄了":return "金液扔给魅魔时，你获得2格金液，并魅惑10回合";
            case "这波不活了":return "你的复活会施加150回合无敌";
            case "露出癖":return "空手+裸衣时，你的综合属性+200%";
            case "抓住未来":return "女人英雄背包每个金液都会+8%综合属性";
            case "诚信互钢":return "你与首次攻击都将额外造成自身6%最大生命的伤害(敌人共享叠钢)，并最大生命永久+12%额外伤害";
            case "灵魂熔炉":return "击杀敌人永久+5最大生命";
            case "升级升级卷轴:自升":return "升级卷轴新增自升按钮，可以使升级永久额外升级1次";
            case "丘比":return "+80魔力，击杀会-1，最多-80";
            case "玻璃大炮":return "最大生命-20%，+20%攻击伤害";
            case "自杀式攻击":return "死亡会对视野内的所有敌人造成自身最大生命的无视防御伤害，对老鬼只有25%效果";
            case "我是Evan":return "最大护甲-50%，魔力+15%，对非相邻敌人的攻击伤害+12%";
            case "大难不死必有后福":return "死亡时会在其位置生成60个随机物品";
            case "摸金校尉":return "翻开坟墓时获得地牢层数x75的金币";
            case "羽落的宠爱":return "获得1瓶羽落秘药，羽落Buff消失时会再次获得1瓶羽落秘药";
            case "豪赌":return "随机3级海克斯，并加入5个没卵用在海克斯组，但是之后的海克斯升级概率/4";
            case "我是色批":return "击杀魅魔会永久+10力量，但是被魅惑时会同时施加麻痹";
            case "玩命":return "你残血以下时，攻击伤害+45%";
            case "细嚼慢咽":return "吃饭时间x1.5，但是饱腹填充x2，以及恢复的生命值x2";
            case "什么档次也配跟我在草丛":return "在草丛且视野内敌人也在草丛则攻击+35%";
            case "狂暴":return "你半血以下时，攻击伤害+20%";
            case "跟着我左手右手一个慢动作":return "切换副武器回合x3，但是每次切换恢复3%最大生命";
            ...
            case "无法刷新海克斯":return "海克斯秘卷无法刷新，但是根据海克斯刷新上限额外获得海克斯秘卷";
            case "高帧战士":return "攻击伤害+50%帧率";
            case "我让你海克斯随机":return "你将无法使用海克斯秘卷获得海克斯，但是你可以用祈愿直接获得对应名字的海克斯并使用掉";
            case "超级加倍":return "可堆叠战利品掉落数量x4";
            case "三国杀:吕布":return "敌人的闪避和防御判定减半";
            ...
            case "三人":return "每回合相邻敌人2时隐形";
            case "读万卷书":return "阅读卷轴永久+5最大生命";
            case "爬楼":return "移速/25%地牢区域，但是防御x50%层数";
            case "泰坦的坚决":return "攻击和防御都会获得层，每层+0.1主属性，1最大防御和最大魔抗，0.01大小，，可以无限叠加，脱离战斗状态会移除所有";
            case "黑暗收割":return "对半血以下的敌人攻击伤害+35，并永久对半血以下敌人5攻击伤害，冷却30回合，击杀会刷新冷却";
            case "我不是药王":return "饮用药剂永久+5最大生命";
            case "强攻":return "攻击一个敌人3次后会+50攻击伤害，并使对其后续攻击伤害+10%";
            case "丛刃":return "攻击会获得10回合的3次+150%攻速，并额外造成该伤害10%的魔法伤害，冷却12回合";
            case "日月星辰":return "综合属性+日期x月份x1%";
            case "持久战":return "攻击-10%，但是战斗状态和击杀状态都会+20%攻击";
            case "魔法箭矢":return "敌人受到伤害会投掷1个箭矢，每个箭矢根据飞行距离最多9%最大生命的魔法伤害";
            case "海克斯获取:经验":return "累计获得25级之后的升级所需经验x6获得海克斯秘卷";
            case "星界躯体":return "+150生命，-10%攻击伤害";
            case "海克斯获取:强制贸易":return "清空金币，每750获得一个海克斯秘卷";
            case "海克斯获取:强制炼金":return "清空能量，每75获得一个海克斯秘卷";
            case "海克斯获取:锻炼":return "获得力量/15数量的海克斯秘卷";
            case "海克斯获取:学习":return "获得魔力/15数量的海克斯秘卷";
            case "我让你饥饿":return "饥饿伤害为0";
            case "肉鸽:武器":return "拾取非武器的可装备的物品时，随机获得武器";
            case "虚空腐蚀":return "战斗状态全能吸血+7%，并且攻击伤害+10%";
            case "米4达":return "除了每第四攻击+44.44%伤害，除了每第四次防御+44.44%物理抗性";
            case "我的眼就算尺度":return "攻击范围为视野范围";
            case "万世催化石":return "受到的10%伤害使其法杖武器神器充能，武器法杖神器使用时恢复20生命";
            case "纯粹主义术师":return "法杖武器神器充能速度+攻速";
            case "穿山甲":return "穿甲+45%最大防御";
            case "连锁闪电":return "攻击回对视野内所有其他敌人造成50%最大攻击的魔法伤害";
            case "海克斯获取:贸易":return "新的商店将出售宝典";
            case "我不是人我只是个怪物":return "每回合1/7概率阅读恐惧卷轴";
            case "高级5阶武器库":return "随机+15等级的三选一5阶武器，可以让5阶武器库也是+15";
            case "健身":return "攻击永久+0.05力量";
            ...
            case "修罗血场":return "+15%吸血，击杀获得的经验根据区域而定而不是怪物本身，刷怪数量x2和刷怪速度x3";
            case "飞机手":return "攻击+30%，但是副武器攻击效果-30%";
            ...
            case "短跑壮如牛长跑瘦如猴":return "饥饿速度x移速，力量/移速，最大生命/移速，大小/移速";
            ...
            case "刷新海克斯":return "海克斯秘卷刷新次数+5";
            case "不是哥们我海克斯呢":return "移除所有海克斯，并根据移除的海克斯数量获得双倍数量的随机海克斯";
            case "夺金":return "首次攻击额外造成40%最大攻击的魔法伤害，并获得金币";
            case "灵感咕力咕力咕力灵感菇":return "踩草必定随机额外获得一个红蓝绿其一蘑菇";
            case "青青子衿悠悠我心":return "最大生命x再生速度倍";
            case "嘟嘟哒嘟嘟哒":return "击杀骷髅会额外掉落2张魅音秘卷";
            case "友好老鬼":return "老鬼的攻击会使你恢复5%最大生命";
            case "护甲是生命":return "护甲恢复也能吃到200%的生命再生";
            case "伤害转嫁":return "你受到的攻击将给视野内所有敌人造成魔法伤害";
            case "海克斯获取:血祭":return "触发复活会获得海克斯秘卷";
            case "力量的护甲":return "最大护甲+力量";
            case "护甲的力量":return "力量+最大护甲";
            case "狂怒":return "你的暴击机制会判定两次";
            case "选牌":return "攻击选其中一张牌的效果，每张牌都是1/3概率，蓝牌(恢复30%已损失护甲)，红牌:炸弹爆炸(不过不会和炸弹一样炸弹英雄以及破坏物品，且伤害减半)，黄牌(施加4回合麻痹)";
            case "日蚀":return "吸血+5%攻速";
            case "眼球收集者":return "击杀区域的BOSS+10%攻击伤害(1~4区域)";
            case "暴伤暴伤":return "溢出的暴击率转暴击伤害100%";
            case "多克特":return "治疗护盾+60%";
            case "吞噬细胞":return "击杀最大生命小于你生命的敌人会永久+3%敌人最大生命的";
            ...
            case "装备融合":return "武器，防具，法杖，戒指可以选择同种物品来融合，使其等级给予自身";
            case "起源:进食":return "击杀敌人获得其3%最大生命和最大攻击";
            case "防御转最大魔抗":return "元素抗性+最大防御/(2.5+最大防御)";
            case "魔抗转防御":return "防御+元素抗性";
            case "升级升级:吸血":return "+0.7%等级全能吸血";
            case "升级缝合像素地牢":return "总综合属性+15%";
            case "即死":return "攻击秒杀生命值低于25%的敌人";
            case "无休回复":return "根据移动距离每格，恢复0.5%最大生命";
            case "攻击Bug":return "攻击伤害+75%，7%概率崩溃并在崩溃前保存游戏";
            case "攻守兼备":return "防御能触发伤害x5的攻击特效";
            case "升级水袋":return "拾取露珠额外获得2露珠";
            case "死亡之环":return "获得治疗同时对2格内敌人造成100%治疗量的伤害";
            case "我的刀盾":return "发现豺狼类型的怪物会使其腐化";
            case "时间之神白野":return "时间停止时，总综合属性+200%";
            case "我不是药神":return "身上每个Buff都会为你提供10%综合属性";
            case "轻装上阵":return "总移速+30%，-20%最大护甲";
            case "巨人杀手":return "大小-0.15，根据与敌人大小差距最多+攻击伤害35%";
            case "混乱":return "攻击以及受到攻击，都会有1/5概率使敌人获得一个6回合的狂乱";
            case "真战士":return "+2%吸血+75最大生命";
            ...
            case "渴血":return "+10%全能吸血";
            case "装备无穷饥渴":return "+3%全能吸血，连杀状态为+12%全能吸血";
            case "关键暴击":return "暴击率+50%";
            case "暴击律动":return "+25%暴击率，暴击时+6%攻速，最多60%";
            case "装备疾射火炮":return "+25%暴击率，+35%攻速，每10回合下次攻击的攻击范围+50%";
            case "砍伤":return "+50%攻速，攻击额外造成50%最大防御的魔法伤害";
            case "超级玻璃大炮":return "-35%最大生命，+40%攻击伤害";
            case "装备无终恨意":return "每5回合对2格敌人造成6%最大生命，并在造成伤害时每个敌人恢复12%最大生命，冷却从武器充能收益";
            case "慢炖":return "每回合对2格内敌人造成0.5%最大生命的无限叠加伤害";
            case "快炖":return "每回合损失1%生命，并对2格内敌人造成1%最大生命的无限叠加伤害";
            ...
            case "动物园园长":return "对动物的攻击伤害+45%";
            case "卧槽炫狗":return "对豺狼类敌人攻击伤害+75%";
            case "海妖杀手":return "对海妖类敌人攻击伤害+100%";
            case "终结者":return "对机械类敌人攻击伤害+75%";
            case "你肩上的恶魔":return "每回合损失0.5%生命，敌人受到的伤害+25%，击杀敌人会掉落治疗药剂，有敌人在视野内时提升至5%生命";
            case "强健体魄":return "最大生命+10倍力量";
            case "正义右拳":return "攻击伤害+3.5x力量";
            case "无极剑道":return "攻击额外造成20%最大攻击的魔法伤害";
            case "大错特错":return "打开的解压，系统，挑战，炼狱，派对，赛季，有10%概率视为关闭";
            case "回归基本功":return "无法使用使用天狗的面具，治疗护盾+45%，攻击伤害+35%，法杖神器武器充能速度+165%";
            case "魄罗爆破手":return "攻击投掷魄罗，攻击伤害+1.5%敌人最大生命的魔法伤害";
            ...
            case "生命的宠爱":return "最大生命+35%英雄防御次数";
            case "防御的宠爱":return "最大防御+7.5%英雄防御次数";
            case "攻击的宠爱":return "最大攻击+15%攻击次数";
            case "Man!":return "力量+80%";
            case "辐射":return "敌人在出生时都会赋予3+60%地牢层数伤害的中毒，如果分离时没有死亡，则该目标的攻击伤害+35%";
            case "升级海克斯":return "获得一个不能刷新的海克斯秘卷，并且海克斯的等级+1";
            ...
            case "不死意志":return "复活后回治疗护盾永久+5%";
            case "更多生命水晶和生命果":return "获得生命水晶x14个，19个生命果可以从草丛中获得";
            case "亮出你的剑":return "攻击范围固定为1，每点额外攻击范围+10%攻速，10%攻击，10%最大生命，5%吸血";
            case "传送":return "你的移动改成传送，但是传送距离为1+30%移速(Boss层仅1)，每次必定消耗1回合";
            case "我还以为减速带呢":return "如果你的移速比相邻敌人大于等于3倍(或者大小大于等于1.5倍)，则秒杀";
            case "贤者之石":return "获得贤者之石，无限使用嬗变卷轴";
            case "海克斯获取:任务":return "完成任务时，获得海克斯秘卷";
            case "海克斯获取:宝藏":return "打开宝藏10%概率获得海克斯秘卷";
            case "无形威胁":return "视野内没有敌人时，下次攻击额外造成50%最大攻击的魔法伤害";
            case "多重施法":return "法杖魔力收益50%概率+100%，25%概率+200%，12.5%概率+400%，6.25%概率+800%";
            case "双发快射":return "暴击时进行1次45%伤害连击，暴击率+25%";
            case "当头一棒":return "攻击1/3概率施加3回合麻痹";
            case "守护灵":return "10次受到伤害-50%。施法+10次";
            case "威慑纪元":return "攻击伤害+35%，但是敌人攻击伤害+15%";
            case "砍倒":return "+50%攻速，攻击额外造成10%最大攻击+最大防御的魔法伤害";
            case "多兰剑":return "最大攻击+10，最大生命+80，吸血+3%";
            case "多兰盔":return "最大防御和最大魔抗+15，最大生命+110";
            ...
            case "Boss阴到没边但是我能回档":return "小老鬼和老鬼的综合属性x8，但是你死亡可以回到45回合之前";
            case "炒能力":return "综合属性+金币/600";
            case "300颗够吗":return "获得300颗坠牢之星";
            case "亮出你的枪":return "攻击范围+6/攻击范围，每超过1攻击范围，则-5%最大生命，+15%攻击，10%攻速";
            case "双刀流":return "总攻速+20%，攻击额外投掷一个无视防御的魔法箭矢，造成40%攻击伤害";
            case "外交豁免":return "攻击目标会5回合内对目标之外的攻击+75%物理抗性，且对目标造成攻击+25%伤害，仅只能有一个目标";
            case "防具的宠爱":return "物理抗性+(防具等级+防具阶级)/((防具等级+防具阶级)+2)";
            case "我开挂了":return "攻击必定秒杀，但是受到伤害x3";
            case "坚固壁垒":return "超过10%生命的所受伤害将减少50%";
            case "恢复的宠爱":return "饮用永生秘药+25%治疗护盾";
            case "不详契约":return "根据已损失生命最多25%移速、20%全能吸血和+150魔力，且每次施法消耗3%生命";
            case "每日属性翻倍":return "总综合属性-45%，每天你的综合属性x2，最多1024倍";
            case "吸收痛苦":return "所有单位受到的伤害都能吸收使你下次攻击额外造成魔法伤害，并消耗25%痛苦";
            case "梦魇":return "恐惧一个敌人时，永久+45力量";
            case "秘术冲拳":return "攻击法杖神器武器充能0.625";
            case "区区致命伤":return "根据已损失生命，获得伤害减免，最多90%";
            case "至死方休:老婆":return "你将受老婆保护，老婆无限命中，无限闪避";
            case "太阳神":return "光照范围+7，击杀敌人1/3概率永久+1光照范围，攻击额外造成光照范围的魔法伤害";
            case "血之饥渴":return "攻击额外造成10%最大攻击的魔法伤害，在半血以下时候同时恢复伤害的生命，在残血以下时恢复的生命x2.5";
            ...
            case "越级挑战":return "敌人生命、攻击和防御、命中和闪避-25地牢层数计算";
            case "最大护甲转生命再生":return "生命再生+1%最大护甲";
            case "无限火力":return "法杖，神器，武器充能速度+350%，总攻速x2，但是治疗护盾-50%";
            case "暴击不暴击":return "未暴击也能+暴击伤害";
            case "装备星蚀":return "2回合内对攻击两次，则对敌人造成10%最大生命的伤害，并获得20%最大攻击护盾，之后需要冷却5回合，冷却从武器充能收益";
            case "物法皆修":return "战斗状态下攻击+0.5力量和魔力，脱离战斗状态移除所有";
            ...
            case "究极玻璃大炮":return "-50%最大生命，攻击伤害+60%";
            ...
            case "重殇":return "攻击伤害+25%攻击次数";
            case "海克斯获取:掉落":return "击杀敌人有1/20概率掉落海克斯秘卷";
            case "你赢了老弟":return "获得Yendor护符";
            case "铜皮铁骨":return "最大防御+450%力量";
            case "飞身踢":return "攻击秒杀生命值低于20%自身最大生命的敌人，并恢复5%最大生命";
            case "红包":return "每回合1/240概率获得红包，红包使用同时会获得一个属性锻造器";
            case "缩小引擎":return "击杀敌人根据强度大小-0.02，以及移速+2%，法杖神器武器充能4%，死亡-65%效果";
            case "坦克引擎":return "击杀敌人根据强度大小+0.025，以及最大生命+5%，死亡-65%效果";
            case "均衡打击":return "攻击伤害减半，但是额外造成另一半的魔法伤害";
            case "战神":return "吸血+15%，最大生命+40%";
            case "法神":return "魔力+150%";
            case "魔法:神代回归":return "法杖魔力收益+350%";
            case "帝王引擎":return "受到严重伤害时恢复75%伤害的生命";
            case "我全都要":return "获得这次所有选项的海克斯";
            case "无敌左手":return "副武器攻击效果+170%";
            case "万剑归宗":return "攻击伤害减半，但是额外攻击剑类武器数量次数";
            case "海克斯获取:真血祭":return "每累计受到225%生命上限，获得海克斯秘卷";
            case "海克斯获取:屠杀":return "每累计125个怪物死亡，获得海克斯秘卷";
            case "升级升级:综合":return "获得20%等级的综合属性";
            case "连拨击锤":return "攻击5次，但是仅50%伤害，上下左右4个方向，每个方向有8回合冷却";
            case "虚空裂隙":return "视野敌人受到伤害会标记6回合，视野内的敌人标记有2个时会消耗其标记，并造成10x(魔力+穿甲+法穿)x(1+护甲穿透+法术穿透)的魔法伤害，每个敌人有3回合冷却标记";
            case "拳拳叠加":return "空手的攻击伤害+35%攻击次数";
            case "我让你打我":return "防御所受伤害为5%，但是受到的伤害翻倍";
            case "战斗久":return "敌人最大生命值x2，但是攻击减半";
            case "无妨我已经看到了未来":return "发现敌人会获得10回合无敌";
            case "装备禁忌时机":return "最大攻击+55，没有战斗状态时，+35穿甲";
            case "升级进阶宝典":return "第二层额外+2天赋点，第三层额外+3天赋点，最大护甲+20%";
            case "量子计算":return "每30秒7格范围的150%最大魔法伤害，3格外的敌人额外造成敌人20%最大生命的魔法伤害，并恢复50%伤害的生命，冷却从神器充能收益";
            case "解除人体限制的诅咒":return "综合属性+每回合+0.1%，你的寿命只有6天，6天爆体而亡(不能复活)";
            case "超凡邪恶":return "击杀敌人时，+0.05魔力";
            case "人不犯我我不犯人":return "战斗状态时+45%综合属性";
            case "我等你":return "综合属性-50%，休息时获得无敌";
            case "升级升级":return "升级武器、防具、法杖、戒指能够额外升级2次";
            case "空手道":return "空手总攻速-50%，攻击+750%";
            case "升级天赋":return "天赋仅需升级一次即满级";
            case "属性叠属性叠":return "击杀敌人1/6概率获得一个属性锻造器";
            ...
            case "我无限回档洞悉所有底牌":return "死亡会复活随机传送，但是地牢将有1/10概率生成的是隐形怪，且当你灵视发现时他们的攻击+5倍，受伤仅20%，命中闪避x5，攻速移速x5";
            ...
            case "狂食鸦群":return "击杀敌人永久+15最大生命";
            case "摸不着头脑":return "视野范围仅1，但是一直无敌";
            case "怪物猎人":return "击杀敌人永久+0.5%攻速和移速";
            case "万魂幡力":return "击杀敌人永久+0.5魔力";
            case "5号化合剂":return "饮用药剂，永久+1%综合属性";
            case "属性叠属性":return "获得3个属性锻造器，且属性锻造器必定锻出最高属性";
            case "牙仙子":return "击杀敌人永久+0.5穿甲和法穿";
            case "天命之子":return "综合属性+2.5倍幸运值";
            case "真连击":return "首次攻击伤害25%的7连击";
            case "广告双倍":return "可堆叠物品可以点击广告双倍按钮，麻痹15秒，并额外获得两份物品";
            case "心之抗":return "首次攻击+0.5最大防御和最大魔抗";
            case "地狱诅咒":return "击杀敌人永久+0.25魔力和最大防御";
            case "升级升级:等级":return "武器、防具、法杖、戒指+英雄等级";
            case "真灵魂虹吸":return "攻击恢复生命，恢复量为10%敌人最大生命";
            case "风雷翅:左风右雷随心意而动":return "总移速x4，移动时对相邻敌人造成移动距离x移速x2的魔法伤害";
            case "591铂金60金15银3铜":return "死亡时获得(591x100x100x100+60x100x100+15x100+3)/5金币";
            case "禁忌物001":return "每个禁忌物为你+20%综合属性x(1+等级)";
            case "芝诺龟":return "敌人与你相邻时，攻速和移速仅25%";
            case "无限材料":return "粘咕球、暗金矿、矮人徽章、邪能碎片物品数量不是0或1，且经过改变时，则该物品数量+2";
            case "阴得没边":return "防御后获得3回合隐形，并必定命中必定暴击";
            case "费米悖论":return "刷怪数量和刷怪速度仅20%，对老鬼+80%物理抗性，你每回合受到10%生命";
            case "海克斯获取:恢复":return "每恢复200%最大生命，获得海克斯秘卷";
            case "智慧末刃:医学奇迹":return "攻击额外造成魔力的魔法伤害，半血以下时攻击恢复魔力的生命";
            case "属性买买买":return "商店出售属性锻造器";
            case "智慧的宠爱":return "消耗能量x12倍，炼金第一次之后的合成不消耗材料";
            case "杀戮之神":return "对非BOSS+50%攻击伤害";
            case "恩赐解脱":return "攻击20%几率使其攻击伤害+4倍";
            case "暗黑破坏神":return "镐子破坏墙体时永久+0.45%综合属性";
    ...
    public void 海克斯触发(String 选择){
        ...
        switch(选择){
            ...
            case "民以食为天":
                for(int x=1;x<=20;x++){
                    Item f=Generator.random(Generator.Category.FOOD);
                    f.放背包();
                }
                return;
            case "海克斯获取:强制贸易":
                for(int x=1;x<=Dungeon.gold/750;x++){
                    new 海克斯秘卷().放背包();
                }
                Dungeon.gold=0;
                return;
            case "海克斯获取:强制炼金":
                for(int x=1;x<=Dungeon.energy/75;x++){
                    new 海克斯秘卷().放背包();
                }
                Dungeon.energy=0;
                return;
            case "海克斯获取:卷轴转换":
                ...
                for(Item i: belongings.backpack){
                    if(i instanceof Scroll a){
                        卷轴数量+=a.数量();
                        a.detachAll();
                    }
                }
                for(int x=1;x<=卷轴数量/5;x++){
                    new 海克斯秘卷().放背包();
                }
                return;
            case "海克斯获取:药剂转换":
                int 药剂数量=0;
                for(Item i: belongings.backpack){
                    if(i instanceof Potion a){
                        药剂数量+=a.数量();
                        a.detachAll();
                    }
                }
                for(int x=1;x<=药剂数量/5;x++){
                    new 海克斯秘卷().放背包();
                }
                return;
            case "海克斯获取:锻炼":
                for(int x=1;x<=力量()/15;x++){
                    new 海克斯秘卷().放背包();
                }
                return;
            case "海克斯获取:学习":

                for(int x=1;x<=魔力()/15;x++){
                    new 海克斯秘卷().放背包();
                }
                return;
            case "贤者之石":
            ...
            case "高级5阶武器库":
                Item wm1;
                    wm1=Generator.randomWeapon(4).升级(符文("高级5阶武器库")?15:5);
                Item wm2;
                do{
                    wm2=Generator.randomWeapon(4).升级(符文("高级5阶武器库")?15:5);
                }while(wm1==wm2);
                Item wm3;
                do{
                    wm3=Generator.randomWeapon(4).升级(符文("高级5阶武器库")?15:5);
            ...
            case "高级戒指库":
                Item r1;
                r1=Generator.randomRing().升级(符文("高级戒指库")?15:5);
                Item r2;
                do{
                    r2=Generator.randomRing().升级(符文("高级戒指库")?15:5);
                }while(r1==r2);
                Item r3;
                do{
                    r3=Generator.randomRing().升级(符文("高级戒指库")?15:5);
                }while(r1==r3||r2==r3);
                Item finalIr=r2;
                Item finalI1r=r3;
                Game.runOnRenderThread(()->{
            ...
            case "我独自升级":
                ...
                return;
            case "武器赌狗":
                Item w= Random.oneOf(Generator.randomWeapon(0),
                                     Generator.randomWeapon(1),
                ...
                return;
            case "防具赌狗":
                Item a= Random.oneOf(Generator.randomArmor(0),
                                     Generator.randomArmor(1),
            ...
            case "属性叠属性":
                ...
                return;
            case "升级海克斯":
                海克斯秘卷 item=new 海克斯秘卷(true);
                item.用过();
                item.放背包();
                return;
            case "任务:海克斯获取":
                海克斯秘卷 item2=new 海克斯秘卷();
                item2.用过();
                item2.放背包();
                return;
            case "你赢了老弟":
    ...
    public void 删除海克斯(String ss){//不能刷到

        if (海克斯表.containsKey(ss)){
            Notes.remove(Notes.findCustomRecord(ss));
            海克斯表.put(ss,百(海克斯表.get(ss))*100+00+0);
        }
    }
    public void 选择海克斯(String ss){
        if(拥有海克斯()>=120){//最多120
            //海克斯限制
            GLog.红("地牢承受不住这么庞大的海克斯数量！" +
                     "\n会崩溃的！所以获得的海克斯未获得。");
            return;
        }
        if(符文("海克斯获取:海克斯获取")&&Random.Int(1)==0)new 海克斯秘卷().放背包();

        if (海克斯表.containsKey(ss)&&!符文(ss)){//没海克斯
            海克斯表.put(ss,百(海克斯表.get(ss))*100+00+1);
        }
        if(符文(ss))
        海克斯触发(ss);
        Sample.INSTANCE.play(Assets.Sounds.海克斯);

        if(符文("好记性如烂笔头")||符文("超级海克斯秘卷")){
        }
        else {
            if(符文(ss)){
    ...
    public List<String> 随机海克斯(int 数量){
        ...
        while (selectedKeys.size() < 数量) {
            String key=随机海克斯(false,-1);
            if (!selectedKeys.contains(key)){//不相同
    ...
    public String 随机海克斯(){
        return 随机海克斯(false,-1);
    }
    public String 选择随机海克斯(){
        String s=随机海克斯(false, -1);
        选择海克斯(s);
        return s;
    }
    public String 选择随机海克斯1(){
        String s=随机海克斯(false, 1);
        选择海克斯(s);
        return s;
    }
    public String 选择随机海克斯2(){
        String s=随机海克斯(false, 2);
        选择海克斯(s);
        return s;
    }
    public String 选择随机海克斯3(){
        String s=随机海克斯(false, 3);
        选择海克斯(s);
        return s;
    }
...
//                    System.out.println("计算出的索引："+权重位+"，集合最大合法索引："+(随机表.size()-1));
                }while(权重位==-1);
//                if(权重位==海克斯权重(s))
                key=随机表.get(权重位);
            }
//            GLog.p(海克斯权重(key)+"");
        ...
        if(select.equals(""))select=随机海克斯(完全随机,随机等级);//重新寻找
    ...
    public boolean 不能海克斯(String 随到){
        ...
        for (Set<String> pair : 不能海克斯) {
            ...
            String a = pairList.get(0);
            String b = pairList.get(1);
            if ((符文(a) && 随到.equals(b)) || (符文(b) && 随到.equals(a))) {
                return true;
            }
        }
        //内部任意两个不同符文都互斥
        Set<String> 不能广告= new HashSet<>() {{
            add("广告复活");
            add("广告双倍");
            add("荣耀升级");
        }};
        for (String 广告 : 不能广告) {
            //我有这个海克斯 随机到了冲突的 随到了不冲突的
            if (符文(广告) && 不能广告.contains(随到) && !广告.equals(随到)) {
                return true;
            }
        }
        ...
        for (String 螺旋 : 不能螺旋) {
            //我有这个海克斯 随机到了冲突的 随到了不冲突的
            if (符文(螺旋) && 不能螺旋.contains(随到) && !螺旋.equals(随到)) {
                return true;
            }
        }

        //内部任意两个不同符文都互斥
        Set<String> 不能宠爱= new HashSet<>() {{
            add("神殿的宠爱");
            add("草木的宠爱");
            add("流水的宠爱");
            add("深渊的宠爱");
        ...
        for (String loveRune : 不能宠爱) {
            //我有这个海克斯 随机到了冲突的 随到了不冲突的
            if (符文(loveRune) && 不能宠爱.contains(随到) && !loveRune.equals(随到)) {
                return true;
            }
        }
        return false;
    ...
    public boolean 是海克斯(String s){
        if(s.equals(""))return false;
        return 海克斯级(s)!=0;
    }
    public int 海克斯级(String sl){
        if(sl.equals(""))return 0;
        if(海克斯表.containsKey(sl))return 百(海克斯表.get(sl));

        return 0;
    }

    public static int 百(int x){

        return x / 100;
    }
    public static int 十(int x){

        return x / 10 % 10;
    }
    public static int 个(int x){

        return x % 10;
    }

    public int 海克斯权重(String sl){
        if(sl.equals(""))return 0;
        if(海克斯表.containsKey(sl)){
            return 十(海克斯表.get(sl));
        }
        return 0;
    }
    ...
    public boolean 符文(String sn){
        if(sn.equals(""))return false;

        if(海克斯表.containsKey(sn))
        return 个(海克斯表.get(sn))==1;

        return false;
    }
    public boolean 删除(String sn){
        if(sn.equals(""))return false;

        if(海克斯表.containsKey(sn))
        return 十(海克斯表.get(sn))==0;

        return false;
    }
    public boolean 伪删除(String sn){
        if(sn.equals(""))return false;

        if(海克斯表.containsKey(sn))
        return 十(海克斯表.get(sn))==1;

        return false;
    }
    public boolean 权重2(String sn){
        if(sn.equals(""))return false;

        if(海克斯表.containsKey(sn))
        return 十(海克斯表.get(sn))==2;

        return false;
    }
    public boolean 权重3(String sn){
        if(sn.equals(""))return false;

        if(海克斯表.containsKey(sn))
        return 十(海克斯表.get(sn))==3;

        return false;
    }
    public boolean 权重4(String sn){
        if(sn.equals(""))return false;

        if(海克斯表.containsKey(sn))
        return 十(海克斯表.get(sn))==4;
...
//        if(Dungeon.daily&&!Dungeon.dailyReplay)幸运++;
        ...
        if(Dungeon.赛季(赛季设置.规则怪谈)){
            if(belongings.weapon!=null&&belongings.weapon.cursed)
                幸运++;
            else 幸运--;
            if(belongings.secondWep!=null&&belongings.secondWep.cursed)
                幸运++;
            else 幸运--;

            if(belongings.armor1()!=null&&belongings.armor1().cursed)
                幸运++;
            else 幸运--;

            if(belongings.armor2()!=null&&belongings.armor2().cursed)
                幸运++;
            else 幸运--;

            if(belongings.misc!=null&&belongings.misc.cursed)
                幸运++;
            else 幸运--;

            if(belongings.misc2!=null&&belongings.misc2.cursed)
                幸运++;
            else 幸运--;

            if(belongings.misc3!=null&&belongings.misc3.cursed)
                幸运++;
            else 幸运--;
            if(belongings.misc4!=null&&belongings.misc4.cursed)
                幸运++;
            else 幸运--;
            if(belongings.misc5!=null&&belongings.misc5.cursed)
                幸运++;
            else 幸运--;
            if(belongings.misc6 != null && belongings.misc6.cursed)
                幸运++;
            else 幸运--;
...
//            }
    ...
    public void 更新生命() {
        ...
        */
        ...
        if(符文("坦克引擎"))最大生命*=1+0.05f*坦克引擎;
        if(符文("古神"))最大生命*=2;
        if(符文("巨型坦克"))最大生命*=1.8f;

        if(符文("亮出你的枪"))最大生命*=1-(攻击范围()-1)*0.05f;
        if(subClass(HeroSubClass.血法师)&&职业精通())最大生命*=1.35f;

        if(符文("玻璃大炮"))最大生命*=0.8f;
        ...
        if(belongings.技能(生命力.class))最大生命*=1.05f+0.05f*belongings.技能等级(生命力.class);

        最大生命*=角斗链枷.减少();
        最大生命*= 1+天赋点数(Talent.心灵之力,0.15f);
        最大生命*= 综合属性()*(1+天赋点数(Talent.躯体强化,0.1f)+天赋点数(Talent.纯极密度,0.15f));

        if(符文("全副武装")||heroClass(HeroClass.机器)){
            最大生命get=最大生命;
            if(符文("全副武装"))
                最大生命*=0.3f;
        }
...
//        生命 = Math.min(生命, 最大生命);//取消生命更新变动
    ...
    public float 更新护甲(){
        ...
        if(heroClass(HeroClass.机器))最大护甲+=最大生命get;

        if(符文("钢铁意志"))最大护甲*=1+根据已损失生命()*3;
        if(符文("轻装上阵"))最大护甲*=1.2f;
        if(符文("我是Evan"))最大护甲*=0.5f;

        if(进阶&&符文("升级进阶宝典"))最大护甲*=1.2F;
        最大护甲*=水晶碎块.护甲();
        最大护甲*=钢壁之戒.最大护甲(this);
        最大护甲*=综合属性()*(1+天赋点数(Talent.躯体强化,0.1f)+天赋点数(Talent.纯极密度,0.15f));
        if(符文("淬体")){
            最大护甲get=最大护甲;
            最大护甲=0;
        }
	...
	public float 魔力() {
        ...
        if(符文("升级升级:魔力"))m+=0.5f*等级;
        if(符文("真以身做法")){
            m+= 最大生命(0.02f);
        }
        if(Dungeon.系统(系统设置.资产之力)){
            m+=Dungeon.energy*0.08f;
        }
        ...
        if(符文("丘比")){
            m+= 80;
            if(hasbuff(击杀累计.class)) m-=buff(击杀累计.class).丘比;
        }
        if(符文("物法皆修")&&hasbuff(战斗状态.class)&&hasbuff(物法皆修.class)){
            m+= buff(物法皆修.class).count/2f;
        }
        if(heroClass(HeroClass.CLERIC))m+=光照范围();
        m+=魔攻之戒.魔力(this);
        ...
        mx+=天赋点数(Talent.奥术专精,0.1f);
        if(种族天赋.equals("巨灵"))mx+=0.25f;
        mx+=魔法飞刀.魔力();
        ...
        if(o instanceof 火球术||
           o instanceof 火墙术){
            if(belongings.hasItem(四叶草法典.class))
                x*=belongings.getItem(四叶草法典.class).魔力收益;
        }
        if(o instanceof 圣光||
           o instanceof 赐福){
            if(belongings.hasItem(神圣法典.class))
                x*=belongings.getItem(神圣法典.class).魔力收益;
        }
        if(o instanceof 痛命||
           o instanceof 死血){
            if(belongings.hasItem(灵月法杖.class))
                x*=belongings.getItem(灵月法杖.class).魔力收益;
        }
        if(o instanceof 符咒||
           o instanceof 掌心雷){
            if(belongings.hasItem(鬼帝钟.class))
                x*=belongings.getItem(鬼帝钟.class).魔力收益;
        }
        if(o instanceof 风刃||
           o instanceof 木遁||
           o instanceof 土遁||
           o instanceof 尘遁){
            if(belongings.hasItem(叛忍护额.class))
                x*=belongings.getItem(叛忍护额.class).魔力收益;
        }
    ...
    public float 力量() {
        ...
        if(符文("人工肌肉"))return 20;
        float str =力量+根骨之戒.strengthBonus(this);
        if(主属性("力量"))str+=主属性();
        else str+=其他属性();
        if(Dungeon.系统(系统设置.资产之力)){
        ...
        if((Holiday.getCurrentHoliday()==Holiday.妇女节||
        符文("043809438"))
           &&女人()){
            strx+=0.1f;
        }
    ...
    public boolean 主属性(String s){
        ...
        if(s.equals("力量")){
            ...
            heroClass(HeroClass.重武)||
            heroClass(HeroClass.行僧)||
            heroClass(HeroClass.近卫)||

            heroClass(HeroClass.兽灵)||
            heroClass(HeroClass.逐姝)||
            heroClass(HeroClass.血鬼)||
            heroClass(HeroClass.来世)
            ){
        ...
        if(s.equals("魔力")){
            if(
                    ...
                    heroClass(HeroClass.道士)||

                    heroClass(HeroClass.机器)||
                    heroClass(HeroClass.戒老)||
                    heroClass(HeroClass.学士)||
    ...
    private static final String 敏捷x = "敏捷";
    ...
    private static final String 暴刹x = "暴刹";
    ...
    private static final String 适应性头盔x = "适应性头盔";
    private static final String 备注x = "备注";
    ...
    protected static final String 化合剂x    = "化合剂";
    ...
    protected static final String 魔抗成长x    = "魔抗成长";
    ...
    protected static final String 法穿成长x    = "法穿成长";
    protected static final String 法术成长x    = "法术成长";
    protected static final String 增加攻击的攻速x    = "增加攻击的攻速";
    protected static final String 增加闪避的移速x    = "增加闪避的移速";
    ...
    public void storeInBundle(Bundle bundle) {
        ...
        bundle.put( 法穿成长x, 法穿成长);
        bundle.put( 法术成长x, 法术成长);

        bundle.put( 增加攻击的攻速x, 增加攻击的攻速);
        bundle.put( 增加闪避的移速x, 增加闪避的移速);
        bundle.put(EXPERIENCE,经验);

        bundle.put(HTBOOST, HTBoost);


        Bundle 海克斯表r = new Bundle();
        for(String s:海克斯表.keySet()){
            海克斯表r.put(s, 海克斯表.get(s));
        }
        bundle.put("海克斯表", 海克斯表r);
    ...
    public void restoreFromBundle(Bundle bundle) {
        ...
        适应性头盔 = bundle.getClass(适应性头盔x);
        备注 = bundle.getString(备注x);
        ...
        法穿成长 = bundle.getFloat( 法穿成长x );
        法术成长 = bundle.getFloat( 法术成长x );
        增加攻击的攻速 = bundle.getFloat( 增加攻击的攻速x );
        增加闪避的移速 = bundle.getFloat( 增加闪避的移速x );
        经验= bundle.getInt(EXPERIENCE);
    ...
    public static void preview(GamesInProgress.Info info, Bundle bundle) {
        ...
        info.备注 = bundle.getString(备注x);
        info.名字 = bundle.getString(名字x);
...
//                    }
    ...
    public float 魔力天赋点数(Talent talent, float x) {
        return 魔力(天赋点数(talent,x));
    }
...
//                    }
    ...
    public void hitSound(float pitch) {
        if (!空手()) {
            ...
            belongings.投掷武器().hitSound(pitch);
            else{
                if(belongings.weapon1()!=null)
                    belongings.weapon1().hitSound(pitch);
    ...
    public boolean blockSound(float pitch) {
        if (belongings.weapon2() != null &&belongings.weapon2().最大防御(this)>=4) {
            Sample.INSTANCE.play(Assets.Sounds.攻击格挡,1,pitch);
            return true;
        }
        if (belongings.weapon2() != null &&belongings.weapon2().最大防御(this)>=4) {
            Sample.INSTANCE.play(Assets.Sounds.攻击格挡,1,pitch);
...
//        回百分比血(0.05f);
        if(符文("疾风步"))回百分比血(0.05f);
        连击.ParryTracker parry = buff(连击.ParryTracker.class);
        if (parry != null) {
            parry.parried = true;
            if (buff(连击.class).getComboCount()<9||!职业精通()) {
                parry.detach();
            }
    ...
    public float 最小防御() {
        ...
        if (belongings.armor1() != null) {
            float armDr = belongings.armor1().最小防御()*(符文("装备大师")?1.15f:1);
            if (力量() < belongings.armor1().力量() && !heroClass(HeroClass.重武)) {
                armDr -= 2 * (belongings.armor1().力量() - 力量());
            }
            if (armDr > 0) dr += armDr;
        }
        if (belongings.armor2() != null) {
            float armDr = belongings.armor2().最小防御()*(符文("装备大师")?1.15f:1)*副防具效果();
            if (力量() < belongings.armor2().力量() && !heroClass(HeroClass.重武)) {
                armDr -= 2 * (belongings.armor2().力量() - 力量());
            }
            if (armDr > 0) dr += armDr;
        }
        if (belongings.weapon1() != null) {
            float wepDr = belongings.weapon1().最小防御(this)*(符文("装备大师")?1.15f:1);
            
            if (力量() < ((Weapon) belongings.weapon1()).力量() && !heroClass(HeroClass.重武)) {
        ...
        if (belongings.weapon2()!=null) {
            ...
            if (力量() < ((Weapon) belongings.weapon2()).力量()&&!heroClass(HeroClass.重武)) {
                wepDr -= 2 * (((Weapon) belongings.weapon2()).力量()-力量());
            }
            if (wepDr > 0) dr += wepDr;
        }
    ...
    public float 增加防御(){
        ...
        if(belongings.armor(ClothArmor.class)&&符文("铁布衫"))x+=5.5;
        if(belongings.armor(ScaleArmor.class)&&符文("龙鳞甲")) x+=2;
    ...
    public float 最大防御() {
        ...
        if(天赋(Talent.双武格挡)){
            ...
            dr+=天赋点数(Talent.双武格挡,0.25f)*
                ((belongings.hasweapon1()?belongings.weapon1().tier():0)
                 +(belongings.hasweapon2()?belongings.weapon2().tier():0))*

                ((belongings.hasweapon1()?belongings.weapon1().强化等级():0)
                 +(belongings.hasweapon2()?belongings.weapon2().强化等级():0));

        }
        if (belongings.armor1() != null) {
            float armDr = belongings.armor1().最大防御()*(符文("装备大师")?1.15f:1);
            if (力量() < belongings.armor1().力量() && !heroClass(HeroClass.重武)) {
                armDr -= 2 * (belongings.armor1().力量() - 力量());
            }
            if (armDr > 0) dr += armDr;
        ...
        if (belongings.armor2() != null) {
            float armDr = belongings.armor2().最大防御()*(符文("装备大师")?1.15f:1);
            if (力量() < belongings.armor2().力量() && !heroClass(HeroClass.重武)) {
                armDr -= 2 * (belongings.armor2().力量() - 力量());
            }
            if (armDr > 0) dr += armDr;
        ...
        if (belongings.weapon1()!=null) {
            float wepDr =belongings.weapon1().最大防御(this)*(符文("装备大师")?1.15f:1);
            if (力量() < belongings.weapon1().力量() && !heroClass(HeroClass.重武)) {
        ...
        if (belongings.weapon2()!=null) {
            ...
            if (力量() < belongings.weapon2().力量()&&!heroClass(HeroClass.重武)) {
                wepDr -= 2 * (belongings.weapon2().力量()-力量());
            }
            if (wepDr > 0) dr += wepDr;
        }
        ...
        if(符文("升级荆棘斗篷"))
        if (thorns != null) {
            dr+=(thorns.itemLevel()+1)/3f*最大护甲;
        }
        if(符文("进攻是最好的防御"))dr+=最大攻击();
        if(符文("升级防具:护甲"))dr+=3*最大护甲;
        if(符文("真穿甲流"))dr+=穿甲();
        if(符文("铁板鞋"))dr+=25;
        if(符文("多兰盔"))dr+=15;
        if(符文("坚固"))dr+=15;
    ...
    public boolean 空手(){
        if(subClass(HeroSubClass.冰魄剑神))return true;
        return 武力之戒.空手(this);
    }
    public boolean 裸衣(){
    ...
    public float 移速() {
        ...
        if (belongings.armor1() != null) {
            speed = belongings.armor1().speedFactor(this, speed);
        }
        if (belongings.armor2() != null) {
            speed = belongings.armor2().speedFactor(this, speed);
        }
        //region +
        float 移速=1;
        ...
        if(天赋(Talent.无声突袭)){
            ...
            for(Mob m:getVisibleEnemies()){
                if(m.surprisedBy(this)){
                    未惊醒=true;
                    break;
                }
            }
            if(未惊醒)移速+=天赋点数(Talent.无声突袭,0.1f);
        }


        if(种族天赋.equals("半人马"))移速+=0.5f;
        if(符文("小猫咪找爸爸")&&heroClass(HeroClass.灵猫)){
        ...
        if(天赋(Talent.窄区突围)){
            int 墙体数量=0;
            for(int i: PathFinder.相邻){
                if(Dungeon.level.在墙体(i+pos))
                    墙体数量++;
            }
            if(墙体数量>5)移速+=天赋点数(Talent.窄区突围,0.15f);
        }
        if(天赋(Talent.快速布阵)){
            移速+=(力量()-10)*属性增幅()/2f*(天赋点数(Talent.快速布阵,0.25f));
        }
        if(裸衣()){
            移速+=(力量()-10)*属性增幅()/2f;
        }
        if(符文("升级狂妄皇冠")&&hasbuff(狂妄皇冠.class)&&hasbuff(连杀状态.class))移速+=0.25f;
        if(符文("刃下生风")){
            移速+=0.1f*穿甲()+护甲穿透();
            移速+=0.1f*法穿()+法术穿透();
        }

        if(符文("罪恶快感")&&hasbuff(罪恶快感.class)&&hasbuff(战斗状态.class)) 移速+=0.5f*buff(罪恶快感.class).count;

        if(符文("鬼影迷踪"))
            移速+= 0.015f*魔力();

        if(符文("不详契约"))
            移速+= Math.min(0.25f,根据已损失生命()*0.25f);


        if(符文("死腿快跑啊"))移速+=根据已损失生命()*0.7f;
        if(符文("铁板鞋"))移速+=0.3f;
        if(符文("缩小引擎"))移速+=0.02f*缩小引擎;
        if(天赋(Talent.浮石冲浪)){
            int 墙体数量=0;
            for (int i : PathFinder.相邻){
                if(Dungeon.level.在墙体(i+pos))墙体数量++;
            }
            移速+=天赋点数(Talent.浮石冲浪,0.06f)*墙体数量;
        }
        if(符文("逃脱者")){
            int b1=0;
            int b2=0;
        ...
        if(符文("踢踏舞")&&hasbuff(踢踏舞.class)){
            移速+= buff(踢踏舞.class).count/10f/2f;
        }
        if(符文("登神长阶"))
        if(等级>=22||(hasbuff(登神长阶.class)&&buff(登神长阶.class).max())){
            移速+= 0.1f;
        }
        if(hasbuff(Invisibility.class))
            移速+=天赋点数(Talent.偷袭姿态,0.15f);
...
//            if(垂直移动||水平移动)speed*=2;
        ...
        if(符文("风雷翅:左风右雷随心意而动"))speed*=4;
        if(符文("爬楼"))speed/=1+Dungeon.区域()*0.25f;
        if(符文("行万里路"))speed*=0.8f;
        if(符文("苦行僧"))speed*=0.85f;
        if(!符文("走完红色路线"))
            speed*=中国国旗.移速();
        ...
        if(符文("急急小子")){
            speed*=1+(能量之戒.weaponChargeMultiplier(this)-1)*0.75f;
            speed*=1+(能量之戒.wandChargeMultiplier(this)-1)*0.75f;
            speed*=1+(能量之戒.artifactChargeMultiplier(this)-1)*0.75f;
        }

        speed*=1+天赋点数(Talent.舞动身法,0.04f);
        speed*= 综合属性()*(1+天赋点数(Talent.下肢强化,0.075f));
        ...
        if(sprite!=null){
            if(momentum!=null){
                ((HeroSprite)sprite).sprint(momentum.freerunning()?
                                                    momentum.speedMultiplier():
                                                    1f);
                speed*=momentum.speedMultiplier();
            }else{
                ((HeroSprite)sprite).sprint(speed);
            }
        }
...
//        speed = AscensionChallenge.modifyHeroSpeed(speed);
    ...
    public boolean canAttack(Char enemy) {
        ...
        if(enemy.hasbuff(Invisibility.class))
            return false;
        if (belongings.weapon() != null) {
            return belongings.weapon().canReach(this, enemy.pos);
        }
            return canReach(this, enemy.pos);
...
//        return 攻击范围(enemy);
    ...
    public float 抗性计算(Class effect){
        HashSet<Class> 抗性表x = new HashSet<>(抗性表);
        return super.抗性计算(effect);
    }

    @Override
    public float 害怕计算(Class effect){
        HashSet<Class> 害怕表x = new HashSet<>(害怕表);
        if (heroClass(HeroClass.灵猫)||heroClass(HeroClass.鼠弟)) {
            害怕表x.addAll(Property.动物.害怕());
        }
        return super.害怕计算(effect);
    }
    ...
    public boolean 免疫(Class effect) {
        ...
        if (heroClass(HeroClass.机器)||种族天赋.equals("机器")) {
            immunes.addAll(Property.机械.免疫());
            immunes.addAll(Property.INORGANIC.免疫());
            immunes.addAll(Property.电.免疫());
        }
        if (heroClass(HeroClass.重武)) {
            immunes.add(Chill.class);
            immunes.add(Frost.class);
        }
        if (belongings.armor(铠甲.class)) {
            immunes.add(Chill.class);
        }
        ...
        if (belongings.armor(祭服.class)) {
            immunes.add(Degrade.class);
        }
        if(belongings.armor(训服.class)){
    ...
    public float 大小(){
        ...
        大小+=大小成长;
        if(符文("泰坦的坚决")&&hasbuff(泰坦的坚决.class)){
            大小+= buff(泰坦的坚决.class).count/100f;
        }
        if(符文("科学狂人"))大小+=科学狂人;
        if(符文("歌利亚巨人"))大小+=0.1f;
        if(符文("古神"))大小+=0.3f;
    ...
    public String 英雄台词(){
        switch (heroClass){
            case WARRIOR: return Random.element(new String[]{
                    "地牢还是不如雪山有趣。在地牢这里没人给我荣誉！",
                    "想当年在雪山，我一杆长枪镇守边疆。",
                    "荣誉！我的剑为荣誉而挥！",
                    "打了半辈子仗，肚子反而更容易饿了。",
                    "在雪山，来的敌人至少是成建制的。",
                    ...
                    "雪山的老伙计们要是看到我，肯定笑我。",
                    "这地牢里，连片像样的雪都找不着。"});
            case MAGE: return Random.element(new String[]{
                    "年迈才学魔法怎么了？天赋和年纪无关。",
                    "这地牢的魔力真浑浊。",
                    "魔杖在手中嗡鸣，它在渴望战斗。",
                    "法杖又满了充能，经验果然是最好的教材。",
                    "年轻人靠体力，老头子靠魔力。",
                    "这把年纪还能进步，这就是天赋。",
                    "当年老师说我学不成了，现在整座塔都闭嘴了。",
                    "这层楼的元素波动不太对劲。",
                    "魔力在流动……附近有法杖的碎片。"});
            case 盗贼: return Random.element(new String[]{
                    "博物馆那件神器还没出手，先来地牢顺点别的。",
                    "轻点……这机关比我偷过的锁都精密。",
                    "顺手牵羊，职业习惯。",
                    "捡东西不费时间，这是我的看家本领。",
                    "墙上那道缝不对劲……后面有暗室。",
                    ...
                    "斗篷一披，我就是这里最安静的人。",
                    "地牢的宝物，属于有本事的那个。"});
            case HUNTRESS: return Random.element(new String[]{
                    "森率里连狼都不敢惹我，地牢的怪物算什么。",
                    "高草丛里有东西在动……我听得见。",
                    "箭在弦上，心在林间。",
                    "猎人从不践踏高草，这是规矩。",
                    ...
                    "在这地下，弓比剑好使。",
                    "稳住呼吸，一箭一个。"});
            case DUELIST: return Random.element(new String[]{
                    "好想找个人痛痛快快打一场！",
                    "勇猛无匹？那是自然。",
                    "谁敢与我决斗！",
                    "越打越有劲，血都热了！",
                    ...
                    "打架还能回血，这买卖划算。",
                    "输了算我的，赢了算剑的。"});
            case CLERIC: return Random.element(new String[]{
                    "愿圣光净化这地牢的污秽。",
                    "恶魔与亡灵，无处可逃。",
                    "光明所至，黑暗退散。",
                    "这里的黑暗，需要更多的光。",
                    ...
                    "恶魔的嚎叫，是它们心虚。",
                    "咒文要念对，净化才有效。"});
            case 巫女: return Random.element(new String[]{
                    "魔法塔的老师看到我背一包诅咒，怕是要气晕。",
                    "诅咒也是力量的一部分。",
                    "这地牢的诅咒浓度……让我如鱼得水。",
                    "又收了一件诅咒，充能速度谢谢它。",
                    "别人怕诅咒，我收藏诅咒。",
                    ...
                    "诅咒取下来了，这就是我的特权。",
                    "塔里的禁书……其实我全看过。"});
            case 重武: return Random.element(new String[]{
                    "符文大陆的风，比这地牢冷多了。",
                    "盾在，人在。",
                    "雪球我都扛得住，还怕这些家伙？",
                    "冻僵？我老家冬天比这冷多了。",
                    ...
                    "扔雪球，是符文大陆的童年必修课。",
                    "免冻体质，怕过谁。"});
            case 镜魔: return Random.element(new String[]{
                    "镜子里的自己，也想活下去。",
                    "一位英雄最后的祈愿，由我来实现。",
                    "击杀……不消耗回合的感觉，真轻。",
                    "镜中影，镜外人，一起活下去。",
                    ...
                    "护甲碎了可以再攒，遗愿不能等。",
                    "镜像一开，谁是真身都未必。"});
            case 道士: return Random.element(new String[]{
                    "此地阴气极重，需谨慎行事。",
                    "恶鬼亡灵，见钟回避。",
                    "本命玉佩在发烫……有邪祟。",
                    "掐指一算……前方有煞。",
                    ...
                    "一柄铜钱剑，斩尽邪祟。",
                    "道可道，非常道；这地牢的地，非常地。"});
            case 行僧: return Random.element(new String[]{
                    "苦行即是修行，地牢亦是道场。",
                    "一无所有，反而轻松。",
                    "步子不停，道心不灭。",
                    "饿一顿不算苦，心里不苦就行。",
                    ...
                    "净化之力护体，邪祟退散。",
                    "流浪四海，何处不是家。"});
            case 近卫: return Random.element(new String[]{
                    "为皇室而战！哪怕在地下。",
                    "近卫军没有逃跑的传统。",
                    "我的长矛，曾护卫过王座。",
                    "王座之后，无人可撼。",
                    ...
                    "阵列不乱，近卫不倒。",
                    "当年站岗的宫殿，比这亮堂。"});
            case 兽灵: return Random.element(new String[]{
                    "我能听见这地牢里生灵的低语。",
                    "连老鼠都在害怕什么东西……",
                    "万物有灵，与我共鸣。",
                    "这附近有野兽的气息，暂无敌意。",
                    ...
                    "能和生灵说话，就不算孤单。",
                    "狼群会懂的，我是个好邻居。"});
            case 机器: return Random.element(new String[]{
                    "核心信号……就在更深处。",
                    "检测到：湿度超标，不适合人类。",
                    "电量充足，目标明确。",
                    "扫描完毕：前方威胁等级中等。",
                    ...
                    "扳手在手，什么都能修，包括自己。",
                    "核心，我找了你很久。"});
            case 女忍: return Random.element(new String[]{
                    "叛逃者的字典里没有回头。",
                    "影子是最可靠的同伴。",
                    "伤越重，我越快。",
                    "额饰还戴着，说明仇还没报完。",
                    ...
                    "越危险的地方，越适合潜行。",
                    "心越静，刀越快。"});
            case 戒老: return Random.element(new String[]{
                    "神戒啊神戒，你到底藏在哪一格？",
                    "我看不见路，但感觉得到暗流。",
                    "指环会指引我的手。",
                    "别出声，让我用耳朵看。",
                    ...
                    "年纪大了，手却稳得很。",
                    "看不见的路，才最好走。"});
            case 逐姝: return Random.element(new String[]{
                    "放逐就放逐，厄运而已，我习惯了。",
                    "这把灵鞭，抽过比地牢可怕的东西。",
                    "厄运缠身？那我就把厄运打趴下。",
                    "又活了一天，厄运先记账上。",
                    ...
                    "厄运之女？我倒觉得自己挺幸运。",
                    "灵鞭一甩，连运气都跟着好。"});
            case 罗兰: return Random.element(new String[]{
                    "十灾将至，众生……涤罪吧。",
                    "饥饿不是障碍，是审判的一部分。",
                    "我即是灾厄。",
                    "第一灾：血。第二灾：蛙。……慢慢来。",
                    ...
                    "饱腹者的傲慢，也该受罚。",
                    "十灾之后，世界会干净些。"});
            case 学士: return Random.element(new String[]{
                    "军训都没这么累，早知道报摄影课了。",
                    "这地牢，够我写三篇论文。",
                    "喝药读卷轴不花时间，就是这天赋。",
                    "毕业论文题目定了：《地牢生态考察》。",
                    ...
                    "干饭的时候谁都打不到我，这很合理。",
                    "随机药剂喝出什么，全看运气，这很科学。"});
            case 灵猫: return Random.element(new String[]{
                    "喵~",
                    "生与死同行，阴阳双生。",
                    "警觉四周！",
                    "喵呜~（白猫蹭了蹭你）",
                    ...
                    "白猫巡逻，黑猫补觉，分工明确。",
                    "猫的第九条命，借你用用。"});
            case 鼠弟: return Random.element(new String[]{
                    "吱————！",
                    "城市的下水道可比这脏多了。",
                    "弩已上弦，吱吱！",
                    "吱吱~（下水道的兄弟们还等我回去吹牛）",
                    ...
                    "地下室的奶酪味……不对，是毒药味。",
                    "城市鼠的生存法则：先跑，再想。"});
            case 凌云: return Random.element(new String[]{
                    "云漂浮着挺好，就是脚下没个实底。",
                    "诅咒？我现在就是诅咒本身。",
                    "低头看看，这云下风景还行。",
                    "风往哪吹，我往哪飘。",
                    ...
                    "被诅咒那天下了雨，现在我就在云里。",
                    "说人话就是：打我的，都疼。"});
            case 血鬼: return Random.element(new String[]{
                    "血的味道……地牢里也很新鲜。",
                    "实验室的门锁，不如这里的结实。",
                    "每滴血，都是我的力量。",
                    "伤口不愈合也好，正好提醒我活着。",
                    ...
                    "吸血刀越用越顺手，像老朋友。",
                    "别靠近我，除非你想捐点血。"});
            case 来世: return Random.element(new String[]{
                    "那个机器人……就藏在这条时间线的深处。",
                    "时间不多了，跑起来。",
                    "未来的风景，也没这里刺激。",
                    "时间线稳定，追捕继续。",
...
//            if (台词 != null) sprite.绿说(台词);
        ...
        //        GLog.p(Random.chances(new float[]{
        //                200000,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,
        ...
        if(紫色心情.概率()>0&&算法.概率学(紫色心情.概率())){
            ...
            if(男人())
                Dungeon.level.dropRandomCell(new LiquidMetal(),pos);
        }
        if(符文("叶绿体"))buff(Hunger.class).吃饭(2);
        if(subClass(HeroSubClass.真人)){
            for(int n: PathFinder.范围3){

                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦开门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "开门物品", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦休门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "休门冻结", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦生门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "生门回血", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦伤门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "伤门魔法", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦杜门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "杜门隐形", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦景门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "景门灵视", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦死门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "死门秒杀", CharSprite.警告橙);
                }
                if(Actor.hasfindChar(pos+n)&&pos+n==pos+PathFinder.八卦惊门){
                    PointF p = DungeonTilemap.raisedTileCenterToWorld(pos+n);
                    FloatingText.show(p.x, p.y, pos+n, "惊门恐惧", CharSprite.警告橙);
                }
            }
        }
        ...
        if(hasbuff(开局属性更新.class)){
            ...
            if(Dungeon.赛季(赛季设置.地牢塔防))护甲=0;

            Buff.detach(this,开局属性更新.class);
        }

        if(subClass(HeroSubClass.冰魄剑神)){
            int x=Math.round(5*(1+天赋点数(Talent.寒冰之境,0.25f)));
            GameScene.add(Blob.seed(pos,x,Freezing.class));
            if(职业精通())
                for (int n : PathFinder.相邻4)
                    if(Actor.findChar(pos+n)==null||!(Actor.findChar(pos+n) instanceof NPC))
                    GameScene.add(Blob.seed(pos+n,x, Freezing.class));
        }
        if(Dungeon.赛季(赛季设置.生化模式)){
            if(Dungeon.地牢时间==时间(9)){
                    ...
                    Music.INSTANCE.play(Assets.Music.生化模式,false);
            }
            if(生化&&!Music.INSTANCE.isPlaying()){
                生化=false;
                for(Mob mob: Dungeon.level.mobs){
                    if(mob.paralysed<=0&&mob.state!=mob.HUNTING){
                        mob.beckon(pos);
                    }
                }
                Dungeon.level.playLevelMusic();//刷新
        ...
        if(符文("过敏性鼻炎")&&算法.概率学(2)){
            力量+=0.02f;
            SPDSettings.震屏强度(4);
            PixelScene.shake( 3, 1 );
            Sample.INSTANCE.play(Assets.Sounds.ROCKS);
        }

        if (符文("暴雨")){
            if(在水中())
                Buff.施加(this,暴雨层.class).set(1);
            else if(hasbuff(暴雨层.class))Buff.施加(this,暴雨层.class).clearCount();
        }
        if (符文("自然即是治愈")&&在草丛())回百分比血(0.05f);
        if (符文("黑暗森林"))
            for(Mob m:getVisibleEnemies())
        ...
        if (符文("无形威胁")&&!视野敌人())
            Buff.施加(this,无形威胁.class);

        if (符文("最终祝福")&&等级 >= 最大等级)
            Buff.延长(this, Bless.class, 1);


        if(在水中()&&符文("升级水:纯水")&&算法.概率学(1/10f))
        Dungeon.level.drop(new Dewdrop(), pos).sprite().drop();

        if(符文("DM-300的宠爱")&&(Dungeon.区域层数(1)||Dungeon.区域层数(2)))
            Buff.施加(this,Invulnerability.class,1);

        if(符文("神圣干预冷却")&&nobuff(神圣干预冷却.class)){
            Buff.延长(this,Invulnerability.class,3);
            Buff.施加(this,神圣干预冷却.class,25/能量之戒.artifactChargeMultiplier(this));
        }
        if(符文("任务:病毒")){
            受伤时(1,Dungeon.class);
            Buff.施加(this,任务病毒.class).set(1);
            if(buff(任务病毒.class).max()){
                属性成长+=0.05f;
                buff(任务病毒.class).clearCount();
            }
        }
        ...
        if(符文("没事就吃浆果")&&算法.概率学(1/150f))
            Buff.刷新(this,Invulnerability.class,1);
...
//        }
        ...
        if (Dungeon.地牢时间>=时间(19.5f)||Dungeon.地牢时间<=时间(4.5f)) {
//            if(true){
            if(算法.概率学(1/450f)&&Dungeon.LimitedDrops.坠牢之星.count<45){
                Dungeon.LimitedDrops.坠牢之星.count++;
                Sample.INSTANCE.play(Assets.Sounds.坠星);

                Dungeon.level.dropRandomCell(new 坠牢之星(),8,pos);
            }
        }
        ...
        if(符文("三人")){
            ...
            if(x>=2)Buff.施加(this,Invisibility.class,2);
        }
        if(符文("当心小蛋糕")&&算法.概率学(3/100f))
            new 蛋糕().放背包();
        ...
        if(符文("装备无终恨意")&&视野敌人()){
            sprite.领域(0x8800FF,2);

            if(nobuff(无终恨意冷却.class))
            ...
            Buff.施加(this,无终恨意冷却.class,5/能量之戒.weaponChargeMultiplier(this));
        }
        if(符文("量子计算")&&视野敌人()){
            sprite.领域(0x8800FF,2);

            if(nobuff(量子计算冷却.class))
            for (int n : PathFinder.范围7){
                Char c= Actor.findChar(pos+n);
                if(c!=null&&c.alignment == Alignment.ENEMY&&Dungeon.level.heroFOV[c.pos]){
                    float d=1.5f*最大攻击()+(c.距离(this)>3?c.最大生命(0.2f):0);
                    c.受伤时(d);
                    回血(d*0.5f);
                }
            }
            Buff.施加(this,量子计算冷却.class,30/能量之戒.weaponChargeMultiplier(this));
        }
        if(符文("快炖")||符文("慢炖")||符文("高压锅")){
            if(符文("快炖"))
                受伤时(生命(0.01f));
            ...
            for (int n : PathFinder.相邻){
                ...
                Char c= Actor.findChar(pos+n);
                if(c!=null&&c.alignment == Alignment.ENEMY&&Dungeon.level.heroFOV[c.pos]){
                    Buff.施加(c,高压锅.class).set(1);
                    c.受伤时(最大生命(0.01f)*c.buff(高压锅.class).count);
                }
            }
...
//                    mob=new Slime();
        ...
        if(heroClass(HeroClass.凌云)
           ||subClass(HeroSubClass.猫头鹰)
           ||种族天赋.equals("巨灵"))flying=true;
...
//        }
    ...
    public float 护甲恢复(){
        ...
        if(heroClass(HeroClass.机器))护甲恢复*=3;

        if(Dungeon.赛季(赛季设置.地牢塔防))护甲恢复=0;
        return 1/150f*护甲恢复;
    }
    public void busy() {
        ready = false;
...
//        }
    ...
    private boolean actOpenChest(HeroAction.OpenChest action) {
        int dst = action.dst;
        if (Dungeon.level.相邻(pos,dst)||pos==dst) {
            path = null;
    ...
    private boolean actUnlock(HeroAction.Unlock action) {
        ...
        if (Dungeon.level.相邻(pos,doorCell)) {
            path = null;
    ...
    private boolean actMine(HeroAction.Mine action) {
        if (Dungeon.level.相邻(pos,action.dst)) {
            path = null;
            if (Dungeon.level.可挖矿(action.dst) && Dungeon.level.insideMap(action.dst)) {
    ...
    private boolean 挖矿处理(int dst,boolean 单挖){
        ...
        for (int i : PathFinder.相邻) {
            if (Dungeon.level.map[dst+i]==Terrain.MINE_CRYSTAL) {
        ...
        for (int i : PathFinder.自相邻) {
            Dungeon.level.discoverable[dst+i] = true;
        }
        for (int i : PathFinder.自相邻) {
            GameScene.updateMap(dst+i);
        }

        final boolean[] 挖了={false};
        ...
        if (crystalAdjacent) {
            sprite.parent.add(new Delayer(0.2f) {
                ...
                protected void onComplete() {
                    ...
                    for (int i : PathFinder.相邻) {
                        if (Dungeon.level.map[dst+i]==Terrain.MINE_CRYSTAL) {
                            Splash.at(dst+i,0xFFFFFF,5);
                            if(Dungeon.branch==1){
                                if(算法.概率学(1/4f))
                                    Level.set(dst+i,Terrain.EMPTY_DECO);
                                else
                                    Level.set(dst+i,Terrain.EMPTY);
                            }
                            else{
                                if(算法.概率学(1/8f))
                                    Level.set(dst+i,Terrain.CHASM);
                                else if(算法.概率学(1/4f))
                                    Level.set(dst+i,Terrain.EMPTY_DECO);
                                else
                                    Level.set(dst+i,Terrain.EMPTY);
    ...
    private boolean actAttack(HeroAction.Attack action) {
        ...
        if (attackTarget.isAlive() && canAttack(attackTarget)) {
    ...
    public void rest(boolean fullRest) {
        ...
        if (!fullRest) {
            if (sprite != null) {
                sprite.showStatus(CharSprite.默认白,Messages.get(this,"wait"));
            }
        }
        resting = fullRest;
    }
    ...
    public int 最大命中(Char target) {
        ...
        float accuracyx=1;
        if (belongings.armor(武服.class)) {
            accuracyx+=.15f;
        }
        accuracyx+= 天赋点数(Talent.顶福精华, 0.13f);

        if(wep==null){
            accuracyx+=0.25f+(力量()-10)*属性增幅();
        }
        ...
        accuracy*=accuracyx*增加命中(target);
                ...
                if(belongings.投掷武器()!=null){
                    ...
                    if(belongings.weapon1()!=null){
                        dmg+=belongings.weapon1().最小攻击()*(符文("装备大师")?
                                                                      1.15f:
                                                                      1);
                        if(belongings.weapon1().拳套())dmg+= 武力之戒.heromin()/2f;
                    }
                    if(belongings.weapon2()!=null){
                        dmg+=belongings.weapon2().最小攻击()*副武器效果()*(符文("装备大师")?
                                                                                   1.15f:
                                                                                   1);
                        if(belongings.weapon2().拳套())dmg+= 武力之戒.heromin()/2f*副武器效果()*(符文("装备大师")?
                                                                                                      1.15f:
                                                                                                      1);
                    }
                }
    ...
    public float 增加攻击(){
        float x=1;
        if(belongings.weapon(炼狱铲.class)){
            x+=0.02f*Dungeon.相对层数();
        }
        if(hasbuff(连杀状态.class))
            x+=暴刹*0.15f;

        if(hasbuff(Invisibility.class))
            x+=天赋点数(Talent.偷袭姿态,0.15f);
        ...
        if(Dungeon.解压(解压设置.偷偷变强))x+=stealth()+1f/惊醒距离();

        if(职业精通()&&subClass(HeroSubClass.狂战士)){
            x+=(Buff.施加(this,怒气.class).怒气+根据已损失生命())*0.00075f;
        }
        if(符文("双手武器专家")&&belongings.weapon!=null&&belongings.weapon.双手())
            x+=0.15f;
        if(符文("什么档次也配跟我在草丛")){
            boolean 敌人在草丛=false;
            for(Mob m:getVisibleEnemies()){
        ...
        if(天赋(Talent.窄区突围)){
            ...
            if(墙体数量>5)x+=天赋点数(Talent.窄区突围,0.1f);
        }
        征服 z=buff(征服.class);
        if(z!=null)x+=z.层数*天赋点数(Talent.战争热诚,0.5f);
        if(heroClass(HeroClass.行僧)){
            x+=移速()*0.15f;
        }
        if(符文("亮出你的枪"))x+=(攻击范围()-1)*0.15f;
        if(hasbuff(连杀状态.class))
            x+=狂妄皇冠.攻击();
        if(subClass(HeroSubClass.戏命师)&&职业精通())x+=暴击率()*0.45f+1f/攻击延迟()*0.35f;
        ...
        if(符文("我身为汽车维修")&&belongings.weapon(修理扳手.class)) x+=6.5f;

        if(符文("道长")&&(belongings.weapon(铜钱剑.class))) x+=6.5f;
        if(符文("站在布隆后面")&&belongings.weapon盾()) x+=0.5f;
        if(符文("我有一剑")&&belongings.weapon剑()) x+=0.3f;
        if(符文("我懂个锤子")&&belongings.weapon锤()) x+=0.4f;
        if(符文("盘古开天")&&belongings.weapon斧()) x+=0.35f;
        if(符文("齐天大圣")&&belongings.weapon棍()) x+=0.45f;
        if(符文("闪电五连鞭")&&belongings.weapon鞭()) x+=0.5f;
        if(符文("磨刀霍霍向猪羊")&&belongings.weapon刀()) x+=磨刀;

        if(belongings.weapon(灵鞭.class)) x+=天赋点数(Talent.灵鞭强化,0.25f);
        ...
        if(belongings.技能(攻击力.class)) x*=1.05f+0.05f*belongings.技能等级(攻击力.class);
        if(符文("风暴")&&连击>0)x*=2;
        if(符文("追上命都给你噢"))x*=移速();
        x*=世界标尺.减少();
        return x;
    ...
    public float 最大攻击() {
                ...
                if(belongings.投掷武器()!=null){
                    ...
                    if(belongings.weapon1()!=null){
                        dmg+=belongings.weapon1().最大攻击()*(符文("装备大师")?
                                                                      1.15f:
                                                                      1);
                        if(belongings.weapon1().拳套())dmg+= 武力之戒.heromax()/2f;
                    }
                    if(belongings.weapon2()!=null){
                        dmg+=belongings.weapon2().最大攻击()*副武器效果()*(符文("装备大师")?
                                                                                   1.15f:
                                                                                   1);
                        if(belongings.weapon2().拳套())dmg+= 武力之戒.heromax()/2f*副武器效果()*(符文("装备大师")?
                                                                                                      1.15f:
                                                                                                      1);
                    }
                }
        ...
        dmg+=天赋点数(Talent.猛攻,0.05f);
        if(符文("多兰剑"))dmg+=10;
        if(符文("宇宙安全声明"))dmg+=80f/视野范围();
        if(符文("荣耀升级"))dmg+=等级;
        if(符文("装备禁忌时机"))dmg+=55;
        if(符文("武器收集家")){
            int 武器数量=0;
    ...
    public float 攻击延迟() {
        ...
        if(符文("登神长阶"))
            if(等级>=22||(hasbuff(登神长阶.class)&&buff(登神长阶.class).max())){
                攻速+= buff(登神长阶.class).count*0.006f*魔力();
            }
        if(符文("六脉神剑")&&
           ((belongings.weapon!=null&&belongings.weapon.剑())
        ...
        if(符文("罪恶快感")&&hasbuff(罪恶快感.class)&&hasbuff(战斗状态.class)) 攻速+=0.3f*buff(罪恶快感.class).count;
        if(符文("砍倒")) 攻速+=0.5f;
        if(符文("丛刃")&&hasbuff(丛刃.class)) 攻速+=1.5f;

        if(hasbuff(暴击律动.class)) 攻速+=0.06f*buff(暴击律动.class).count;

        if(符文("狂热者")) 攻速+=0.03f*魔力();
        ...
        if(符文("亮出你的枪"))攻速+=(攻击范围()-1)*0.1f;
        if(重生怪物.equals("矮人武僧"))
            攻速+=1;
        if(符文("装备疾射火炮"))攻速+=0.35f;
        if(符文("砍伤"))攻速+=0.5f;
        if(符文("帝星飘摇荧惑高"))攻速+=0.5f;
        if(符文("属性转换"))攻速+=移速();
        if(第x次攻击%2==0){
            if(符文("左拳伤害高右拳高伤害"))
                攻速+=0.15f;
        ...
        if (天赋(Talent.连击强化)) {
            攻速+=Buff.施加(this, 连击.class).count*天赋点数(Talent.连击强化,0.05f);
        }
        ...
        if(天赋(Talent.追捕猎物)){
            ...
            if(残血)
                攻速+=天赋点数(Talent.追捕猎物,0.3f);
            else if(半血)
                攻速+=天赋点数(Talent.追捕猎物,0.15f);
        }
        征服 z=buff(征服.class);
        ...
        delay/=综合属性()*(1+天赋点数(Talent.手臂强化,0.1f));

        if(天赋(Talent.符文势力)&&hasbuff(战斗状态.class))delay/=1+天赋点数(Talent.符文势力,0.2f);
        delay/=1+天赋点数(Talent.舞动身法,0.075f);
        if(符文("双刀流"))delay/=1.2f;
        if(符文("风暴")&&连击>0)delay/=2;
        if(符文("闪电打击"))delay/=1.2f;
        if(符文("无限火力"))delay/=2;
        if(符文("空手道"))delay*=2;
        if(subClass(HeroSubClass.戏命师))delay/=0.65f;

        delay/=狂怒之戒.attackSpeedMultiplier(this);
        if(天赋(Talent.无声突袭)){
            boolean 未惊醒=false;
            for(Mob m:getVisibleEnemies()){
                if(m.surprisedBy(this)){
            ...
            if(未惊醒)delay/=1+天赋点数(Talent.无声突袭,0.2f);
        }
        //endregion
    ...
    public float 暴击率(){
        ...
        if(英精英雄==8)暴击率+=0.3f;
        if(符文("迅雷不及掩耳"))暴击率+=0.01f*敏捷();
        if(符文("未知防御"))暴击率+=0.25f;
        if(符文("双手武器专家")&&belongings.weapon!=null&&belongings.weapon.双手())暴击率+=0.15f;

        if(heroClass(HeroClass.罗兰)){
            if(hasbuff(WellFed.class))暴击率+=Math.min(0.15f,
                                                       (float)buff(WellFed.class).left/
                                                       (float)buff(WellFed.class).上限()*0.15f);
            if(hasbuff(Hunger.class))暴击率+=Math.min(0.15f,
                                                      (Hunger.STARVING-buff(Hunger.class).level)/
                                                      Hunger.STARVING*0.15f);

        }
        暴击率+=角斗链枷.暴击率()*根据已损失生命();
        if(种族天赋.equals("兽人"))暴击率+=0.2f;
        if(符文("升级升级:暴击")) 暴击率+=0.03f*等级;
        if(符文("鹰头猫")) 暴击率+=0.04f*视野范围();
        if(符文("致命")) 暴击率+=0.1f;
        if(符文("狂怒攻击")) 暴击率+=0.25f;
        if(符文("双发快射")) 暴击率+=0.25f;
        if(符文("狂热者")) 暴击率+=0.03f*魔力();
        if(符文("暴击率的宠爱")) 暴击率+=第x次攻击*0.001f;
        if(符文("战斗狂怒")) 暴击率+=根据已损失生命()*1;
        if(符文("装备疾射火炮")) 暴击率+=0.25f;
        if(符文("浪客之道")) 暴击率+=0.25f;
        if(符文("暴击律动")) 暴击率+=0.25f;
        if(符文("临终轻语")) 暴击率+=0.25f;
        if(符文("关键暴击")) 暴击率+=0.5f;
        if(符文("会心治疗")) 暴击率+=0.25f;
        if(符文("会心闪避")) 暴击率+=0.25f;
        if(符文("灵魂虹吸")) 暴击率+=0.25f;
        if(符文("珠光护手")) 暴击率+=0.25f;
        if(符文("易损")) 暴击率+=0.25f;
        if(符文("会心防御")) 暴击率+=0.25f;
        if(符文("升级无尽之刃")) 暴击率+=0.25f;
        暴击率+=暴率成长;
        if (天赋(Talent.洪荒之怒)) {
            怒气 怒气= Buff.施加(this,怒气.class);
            暴击率+=怒气.怒气*天赋点数(Talent.洪荒之怒,0.0015f);
        }
        暴击率+=角斗链枷.暴击率()*根据已损失生命();
        暴击率+=天赋点数(Talent.破绽,0.03f);
        暴击率+=天赋点数(Talent.战斗本能,0.075f);
        if(heroClass(HeroClass.近卫)){
            暴击率+=0.06f;
        }
        if(belongings.weapon(龙牙锤.class)){
            暴击率+=0.45f;
        }
        if(belongings.weapon(无尽之刃.class)){
            暴击率+=0.18f;
        }
        if (在草丛()) {
            暴击率+=天赋点数(Talent.丛林法则,0.12f);
        }

        if(符文("浪客之道")) 暴击率*=2.5f;
        if(符文("瞄准脑袋")) 暴击率=Math.min(0.5f,暴击率);
        return 暴击率;
    }
    @Override
    public float 暴击伤害(){
        ...
        float 暴击率判定=暴击率();
        if(暴击率判定>(符文("瞄准脑袋")?0.5f:1)){
            if(符文("暴伤暴伤"))
                暴击伤害+=暴击率判定;
            else 暴击伤害+=暴击率判定/3f;
        }

        if(heroClass(HeroClass.近卫))暴击伤害+=0.05f;
        if(符文("双手武器专家")&&belongings.weapon!=null&&belongings.weapon.双手())暴击伤害+=0.15f;
        if(符文("给我暴！")) 暴击伤害+=0.85f;
        if(符文("致命")) 暴击伤害+=0.1f;
        if(符文("原神:不屈")) 暴击伤害+=0.5f;
        if(hasbuff(战斗状态.class))
            暴击伤害+=天赋点数(Talent.创造灾难,0.25f);

        if(belongings.weapon(无尽之刃.class)){
            暴击伤害+=0.3f;
        }
        if(subClass(HeroSubClass.戏命师))
            暴击伤害-=0.1f;

        暴击伤害+= 暴伤成长;

        if(符文("严厉斥责"))暴击伤害+=Random.Float(暴击率判定);
        if(符文("升级无尽之刃")&&belongings.weapon instanceof 无尽之刃)暴击伤害+=暴击率判定/2f;
    ...
    public float 暴击判定(final Char enemy,float dmg){
        ...
        if(enemy!=null&&hasbuff(Invisibility.class)){
            if(belongings.weapon instanceof 变态刀)
                Buff.施加(this,必定暴击.class);
        }
        if(enemy!=null&&符文("内部恐怖")){
            if(enemy.相邻(this)){
                dmg*=1.55f;
                Buff.施加(this,必定暴击.class);
            }else dmg/=2;
        ...
        if(暴击了){
            if(空手()&&符文("认真的一拳"))dmg*=2;

            if(暴击率判定(1/2f)&&符文("双重暴击"))
                dmg*=1+暴击伤害();
            if(暴击率判定(1/3f)&&符文("三重暴击")){
                dmg*=1+暴击伤害();
                dmg*=1+暴击伤害();
            }
            if(符文("暴击律动")) Buff.施加(this,暴击律动.class).set(1);
            if(符文("灵魂虹吸")) 回血(dmg*0.125f);

            if(enemy!=null){
                if(符文("麒麟骨"))Buff.施加(this,麒麟骨.class,5);

                if(符文("爆伤"))new Bomb.ConjuredBomb().heroexplode(enemy.pos);

                if(subClass(HeroSubClass.戏命师)&&职业精通())Buff.施加(this,极速.class,1f);
                ...
                if(enemy.isAlive()){
                    ...
                    if(符文("初生之誓"))enemy.受伤时(0.01f*魔力()*dmg);
                    if(符文("冰霜之书"))算法.修复效果(()->{
                        Buff.施加(enemy,Frost.class,Frost.DURATION);
                    });
                    if(符文("赐死剑气")){
                        if(hasbuff(赐死剑气冷却.class)){
                            buff(赐死剑气冷却.class).spendConstant(2);
                        }
                    }
                }
            }
...
//        GameScene.flashForDocument(Document.ADVENTURERS_GUIDE, Document.GUIDE_INTRO);
        ...
        if(belongings.投掷武器()!=null){
            damage=belongings.投掷武器().攻击时(this,enemy,damage);
        }else{
            if(belongings.weapon1()!=null)
                damage=belongings.weapon1().攻击时(this,enemy,damage);

            if(belongings.weapon2()!=null)
                damage=belongings.weapon2().攻击时(this,enemy,damage);
        }
        if (enemy!=null)
        damage=攻击特效(enemy,damage);

        damage=攻击伤害(enemy,damage);
    ...
    private float 攻击伤害(Char enemy,float damage){
        //region x
        float 伤害倍率=攻击效果();
        ...
        if(enemy!=null&&符文("安内")){
            ...
            if(enemy!=null&&!攻击范围(enemy))
            伤害倍率+=神射之戒.levelBonus(this);
        ...
        if(蛇皮走位==100&&符文("第一滴血"))伤害倍率+=0.45f;

        if(符文("恩赐解脱")&&算法.概率学(1/4f))伤害倍率+=4;

        if(符文("雷霆大A"))伤害倍率+=0.25f;

        if(enemy!=null&&符文("孤立无援")&&enemy.孤立无援())伤害倍率+=0.45f;

        if(enemy!=null&&belongings.充满技能(三重爪击.class))
            伤害倍率+=1+belongings.技能等级(三重爪击.class)*0.5f;

        if(enemy!=null){
            if(belongings.hasItem(风斩电刺.class)){
                if(belongings.充满技能(风斩电刺.class))
                    伤害倍率+=0.75f+belongings.技能等级(风斩电刺.class)*0.25f;
                else
                    伤害倍率+=-0.5f+belongings.技能等级(风斩电刺.class)*0.25f;
            }
        }

        if(enemy!=null&&enemy.恶魔()&&符文("黄金忍者")&&(
                belongings.hasItem(火焰剑.class)&&
                belongings.hasItem(地裂镰.class)&&
                belongings.hasItem(寒冰镖.class)&&
                belongings.hasItem(闪电双截棍.class)
        ...
        if(符文("攻击Bug")){
            伤害倍率+=0.75f;
            if(enemy!=null&&算法.概率学(7)){
                Dungeon.保存游戏();
                throw new RuntimeException("攻击Bug");
            }
        }
        ...
        if(enemy!=null&&enemy.第x次防御>1){
            伤害倍率+=天赋点数(Talent.佯攻,0.08f);
        }
        if(enemy!=null&&符文("高倍望远镜")){
            伤害倍率+=Math.min(0.4f,0.1f*距离(enemy));
        }
        if(符文("强攻")&&enemy!=null&&enemy.第x次防御>3) 伤害倍率+=0.1f;

        if(符文("英雄平衡性增益")) 伤害倍率+=0.4f*根据已损失生命();
        if(符文("虚空腐蚀")&&hasbuff(战斗状态.class)) 伤害倍率+=0.1f;
        if(符文("威慑纪元"))伤害倍率+=0.35f;
        if(符文("回归基本功"))伤害倍率+=0.3f;
        if(符文("眼球收集者")){
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_1))伤害倍率+=0.1f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_2))伤害倍率+=0.1f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_3))伤害倍率+=0.1f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_4))伤害倍率+=0.1f;
        }
        if(符文("来吧老弟"))伤害倍率+=0.2f;
        if(符文("战场Boss")&&Dungeon.bossLevel())伤害倍率+=0.2f;
        if(符文("最终Boss")&&Dungeon.bossLevel())伤害倍率+=0.45f;
        if(符文("星界躯体"))伤害倍率-=0.1f;
        if(符文("玩命")&&残血())伤害倍率+=0.45f;
        if(符文("狂暴")&&半血以下())伤害倍率+=0.2f;
        if(符文("伤害伤害"))伤害倍率+=0.15f;
        if (enemy!=null&&heroClass(HeroClass.HUNTRESS)&&视野范围(enemy)) {
            伤害倍率+=0.08f;
        }
        if(符文("玻璃大炮"))伤害倍率+=0.2f;
        if(符文("超级玻璃大炮"))伤害倍率+=0.4f;
        if(符文("究极玻璃大炮"))伤害倍率+=0.6f;
        if(符文("猛攻"))伤害倍率+=0.5f*(吸血()+全能吸血());


        if(第x次攻击%4!=0){
            if(符文("米4达"))伤害倍率+=0.4444f;
        }
        伤害倍率+=攻击范围()*天赋点数(Talent.蛇腹剑刺,0.075f);
        if(enemy!=null&&subClass(HeroSubClass.猫头鹰)){
            if(感知范围(enemy))
                伤害倍率+=0.15f;
            if(视野范围(enemy))
                伤害倍率+=0.15f;
        }
        if(enemy instanceof Mob m&&符文("什么档次也配和我同等级")&&m.最大等级==等级){
            伤害倍率+=1f;
        }
        ...
        if(enemy!=null&&Dungeon.符文("怕光")&&视野范围(enemy)) 伤害倍率+=0.1f;
        if(enemy!=null&&符文("动物园园长")&&enemy.动物()) 伤害倍率+=0.45f;
        if(enemy!=null&&符文("海妖杀手")&&enemy.海妖()) 伤害倍率+=1f;
        if(enemy!=null&&符文("终结者")&&enemy.机械()) 伤害倍率+=0.75f;
        ...
        if(enemy!=null&&符文("卧槽炫狗")){
            if(enemy.豺狼()) 伤害倍率+=0.75;
        }
        ...
        if(enemy!=null&&符文("我是Evan")&&!相邻(enemy)) 伤害倍率+=0.12f;

        伤害倍率+=天赋点数(Talent.致命华彩,0.075f)*攻击范围();
        ...
        if(enemy!=null&&天赋(Talent.狙击弱点)){
            伤害倍率+=距离(enemy)*天赋点数(Talent.狙击弱点,0.05f);
        }
        ...
        if(enemy!=null&&天赋(Talent.盈能打击)){

            float x=0;
            for(Item i: belongings.backpack){
                if(i instanceof Wand w){
                    x+=w.curCharges*天赋点数(Talent.盈能打击,0.25f)/10f;
                }
            }
            伤害倍率+=Math.min(天赋点数(Talent.盈能打击,0.25f),x);
        ...
        if(enemy!=null&&骸骨左轮.伤害()>0){
            damage+=enemy.最大生命(骸骨左轮.伤害());

            if(骸骨左轮.减少()<0){
                受伤时(生命(骸骨左轮.减少()),骸骨左轮.class);
            }
        }
        if (enemy!=null&&符文("纪元")&&hasbuff(被发现.class))
            damage+=enemy.buff(被发现.class).visualcooldown()*5;


        if(符文("黑暗收割")&&enemy!=null&&enemy.半血以下()&&nobuff(黑暗收割冷却.class)){
            Buff.施加(this,黑暗收割.class).set(5*攻击效果());
            Buff.施加(this,黑暗收割冷却.class,30);
            damage+=buff(黑暗收割.class).count;
        }
        if(符文("强攻")&&enemy!=null&&enemy.第x次防御==3){
            damage+=50f;
        }
        if(符文("高帧战士")){
            damage+=SPDSettings.游戏帧率()*30/2f;
        }
        if(符文("燃血法")){
            damage+=生命(0.1f);
        }
        if(符文("狂怒攻击")) damage+=暴击率()*100*暴击伤害();//攻击伤害改
        if(符文("神射法师")) damage+=魔力(belongings.weapon,0.75f);
        ...
        if(第x次攻击%4==0||天赋概率(Talent.完美谢幕,25))
            if(enemy!=null&&subClass(HeroSubClass.戏命师)){
                Buff.施加(this,必定暴击.class);
                damage+=enemy.已损失生命(0.15f)+enemy.最大生命(天赋点数(Talent.完美谢幕,0.02f));
            }

        if(enemy!=null&&belongings.hasItem(无畏冲锋.class)&&enemy.第x次防御==1)
            Buff.施加(this, Swiftthistle.TimeBubble.class).reset(1+belongings.技能等级(无畏冲锋.class));

        if(enemy!=null&&belongings.hasItem(新月护卫.class)&&enemy.第x次防御==1)
            damage+=enemy.生命(0.1f+belongings.技能等级(新月护卫.class)*0.05f);
    ...
    public float 副武器效果(){
        float x=0.3f+天赋点数(Talent.复合损伤,0.1f);
        if(符文("左撇子"))
            x+=0.7f;
        if(符文("飞机手"))
            x-=0.3f;
    ...
    private float 攻击特效(Char enemy,float damage){
        ...
        if(enemy!=null&&符文("登神长阶")&&(等级>=22||hasbuff(战斗状态.class))){
            Buff.施加(this,登神长阶.class).set(攻击效果());

            if(等级>=8||buff(登神长阶.class).max()){
                for (int n : PathFinder.相邻){
                    Char c= Actor.findChar(pos+n);
                    if(c!=null){
                        c.受伤时(damage*0.2f*攻击效果());
                    }
                }
            }
        }
        if(enemy!=null&&符文("不灭之握")&&enemy.isAlive()&&nobuff(不灭之握冷却.class)){
            Buff.施加(this,不灭之握冷却.class,5);
            生命成长+=5;
            enemy.受伤时(最大生命(0.04f));
            回百分比血(0.02f);
        }
        if(enemy!=null&&天赋(Talent.咒刃))
        enemy.受伤时(魔力(天赋点数(Talent.咒刃,0.2f))*攻击效果());
        ...
        if(enemy!=null&&天赋(Talent.充能护甲)){

            float x=0;
            for(Item i: belongings.backpack){
                if(i instanceof Wand w){
                    x+=w.curCharges*天赋点数(Talent.充能护甲,0.1f)/10f;
                }
            }
            回百分比护甲(Math.min(天赋点数(Talent.充能护甲,0.1f),x));

        }
        if(enemy!=null&&英精英雄==0)
            Buff.施加(enemy,火毒.class).reignite(enemy);

        if(enemy!=null&&英精英雄==6&&算法.概率学(15))
            算法.修复效果(()->{
        ...
        if (enemy!=null&&符文("真灵魂虹吸")){
            回血(enemy.最大生命(0.1f*攻击效果()));
        }
        if (enemy!=null&&subClass(HeroSubClass.神兽之灵)){
            if(enemy.第x次防御==1){
                Buff.延长(enemy,Paralysis.class,2);
            }
        }
        if (enemy!=null&&符文("贯穿星辰")){
            Ballistica b=new Ballistica(pos,enemy.pos,Ballistica.IGNORE_SOFT_SOLID);

            for (int i : b.subPath(2, b.dist)){
                Char ch = Actor.findChar( i );
                if (ch != null&&ch.isAlive())
                    ch.受伤时(damage*攻击效果(),this);
            }
        }
        if (enemy!=null&&subClass(HeroSubClass.不灭战士)&&hasbuff(战斗状态.class)) {
            生命成长+=天赋点数(Talent.不灭之魂,0.05f)*攻击效果();
            攻击成长+=天赋点数(Talent.怀恨在心,0.025f)*攻击效果();

        }
        if(enemy!=null&&Dungeon.系统(系统设置.攻击成长)){
            攻击成长+=Dungeon.depth/100f*攻击效果();
        }

        if(enemy!=null&&符文("健身")) 力量+=0.05f*攻击效果();
        if(enemy!=null&&符文("光盾打击")&&nobuff(光盾打击冷却.class)){
            Buff.施加(this,必定暴击.class);

            Buff.施加(this,光盾打击冷却.class,10f/能量之戒.weaponChargeMultiplier(this) );
            回已损失血(0.15f);
        }
        if(enemy!=null&&符文("丛刃")&&nobuff(丛刃冷却.class)){
            Buff.施加(this,丛刃.class,12f );
            Buff.施加(this,丛刃冷却.class,12f );
        }
        if(enemy!=null&&距离(enemy)>3&&符文("要射金液了")){
            new LiquidMetal().数量(Math.round(攻击效果())).放背包();
        }
        if(enemy!=null&&符文("雷霆腰带")){
            防御成长+=0.075f*攻击效果();
            魔抗成长+=0.075f*攻击效果();
        }
        if(enemy!=null&&enemy.第x次防御==1&&符文("心之疗")){
            治疗成长+=0.3f;
        }
        if(enemy!=null&&enemy.第x次防御==1&&符文("心之力")){
            力量+=0.2f;
        }
        if(enemy!=null&&enemy.第x次防御==1&&符文("心之抗")){
            防御成长+=0.5f;
            魔抗成长+=0.5f;
        }
        if(enemy!=null&&enemy.第x次防御==1&&符文("给你打成折叠屏")){
            enemy.折叠屏=true;
        }


        if(enemy!=null&&符文("吸元秘术")){
            if(hasbuff(吸元秘术冷却.class)){
                buff(吸元秘术冷却.class).spendConstant(((enemy.老鬼()||enemy.小老鬼())?4.5f:3)*攻击效果());
            }else{
                Buff.施加(this,吸元秘术冷却.class,30);
                回血(最大生命(0.1f*攻击效果()));
            }
        }

        if(enemy!=null&&符文("赐死剑气")&&enemy.isAlive()){
            if(hasbuff(赐死剑气冷却.class)){
                buff(赐死剑气冷却.class).spendConstant(2*攻击效果());
            }else{
                Buff.施加(this,赐死剑气冷却.class,20);
                回血(enemy.最大生命(0.1f));
                enemy.受伤时(enemy.最大生命(0.1f));
            }
        }
        ...
        if(enemy!=null&&符文("凡灵双剑")){
            enemy.受伤时(damage*0.25f*攻击效果());
        }

        if(enemy!=null&&符文("无形威胁")&&hasbuff(无形威胁.class)){
            enemy.受伤时(damage/2f);
            Buff.detach(this,无形威胁.class);
        }

        if(enemy!=null&&符文("智慧末刃:医学奇迹")){
            enemy.受伤时(魔力()*攻击效果());
            if(半血以下())回血(魔力()*攻击效果());
        }

        if(enemy!=null&&符文("丛刃")&&hasbuff(丛刃.class)){
            buff(丛刃.class).set(-1);
            enemy.受伤时(damage/10f);
        }

        if(enemy!=null&&符文("附魔剑")&&enemy.isAlive())
            enemy.受伤时(damage*奥术之戒.enchantPowerMultiplier(this)*攻击效果());

        if(enemy!=null&&符文("砍倒")&&enemy.isAlive()) enemy.受伤时((0.1f*最大攻击()
                                                                     +最大防御())*攻击效果());
        if(enemy!=null&&符文("痛苦")&&enemy.isAlive()) enemy.受伤时(50*死亡次数*攻击效果());

        if(enemy!=null&&belongings.充满技能(果决攻击.class))
            回百分比血((1+belongings.技能等级(果决攻击.class))*0.02f*攻击效果());

        if(符文("吸收痛苦"))
        if (enemy!=null&&buff(吸收痛苦.class)!=null) {
            if(enemy.isAlive())
                enemy.受伤时(buff(吸收痛苦.class).痛苦()*攻击效果(),吸收痛苦.class);
        }

        if (enemy!=null&&天赋(Talent.MYSTICAL_CHARGE)){
            ArtifactRecharge.chargeArtifacts(this,天赋点数(Talent.MYSTICAL_CHARGE,0.25f)*攻击效果());
            Weapon.chargeWeapons(天赋点数(Talent.MYSTICAL_CHARGE,0.25f)*攻击效果());
        }

        if(enemy!=null&&Dungeon.符文("装备星蚀")&&nobuff(星蚀冷却.class))Buff.施加(this,星蚀.class,2).set(攻击效果(),enemy);

        if(enemy!=null&&subClass(HeroSubClass.圣女))Buff.施加(enemy,仙法.class,3).set(攻击效果(),this);
        ...
        if(enemy instanceof Mob m&&m.第x次防御==1&&符文("拿来吧你")){
            if(m.loot!=null){
                m.createLoot().数量(Math.round(攻击效果())).放背包();
            }
        }
        if(enemy!=null&&符文("秘术冲拳")){
            Weapon.chargeWeapons(0.625f*攻击效果());
            Recharging.chargeWands(0.625f*攻击效果());
            ArtifactRecharge.chargeArtifacts(this,0.625f*攻击效果());
        }
        ...
        if(enemy!=null&&天赋(Talent.荣耀之证)){
            回百分比护甲(天赋点数(Talent.荣耀之证,0.075f*攻击效果()));
        }
        if(enemy!=null&&天赋(Talent.荣耀之证)){
            回百分比护甲(天赋点数(Talent.荣耀之证,0.075f*攻击效果()));
        }
        if(enemy!=null&&天赋(Talent.杀意波动))回血(感知范围()*天赋点数(Talent.杀意波动,0.35f)*攻击效果());
        ...
        if(enemy!=null&&subClass(HeroSubClass.真人)){
            ...
            if(触发&&天赋(Talent.金光护罩))Buff.施加(this,圣盾.class).增加(天赋点数(Talent.金光护罩));
        }
        if(enemy!=null&&符文("命中的宠爱")) 命中成长+=0.5f*攻击效果();
        if(enemy!=null&&符文("毒素"))
        Buff.施加(enemy,Poison.class).set(damage*0.45f);
        ...
        if(enemy!=null&&visibleEnemies()>1&&subClass(HeroSubClass.神兽之灵)){
            ...
            for(Mob m:getVisibleEnemies()){
                if(m.alignment==Alignment.ENEMY&&m!=enemy){
                    最多攻击++;
                    if(最多攻击>1+天赋点数(Talent.极凌飓风))break;
                    if(m.isAlive())m.受伤时(damage*攻击效果()*(0.35f+天赋点数(Talent.极凌飓风,0.1f)));
                }
            }
        }
        ...
        if(enemy!=null&&subClass(HeroSubClass.金刚独狼))
            回已损失血((0.005f+天赋点数(Talent.嗜血如故,0.005f)+(职业精通()?0.005f:0))*攻击效果());


        if (enemy!=null&&(enemy instanceof Rat||enemy instanceof Piranha)&&heroClass(HeroClass.灵猫)) {

            Buff.施加(this,必定命中.class);
            Buff.施加(this,必定暴击.class);
        }

        if (enemy!=null&&damage>0&&subClass==HeroSubClass.狂战士) {
           Buff.施加(this,怒气.class).damage(攻击效果());
        }
        ...
        if(enemy!=null&&enemy.isAlive()&&符文("血之饥渴")){
            ...
            if(半血以下())
                ...
                回血(0.1f*最大攻击()*2.5f*攻击效果());
        }
        if(enemy!=null&&符文("附魔神剑")){
            for(Item i: belongings.backpack){
                if(i instanceof Weapon w&&w.hasEnchant()){
                    if(buff(MagicImmune.class)==null){
                        if(w.enchantment!=null){
                            damage=w.enchantment.proc(w,this,enemy,damage);
                        }
                    }
                }
            }
        ...
        if(enemy!=null&&enemy.isAlive()&&符文("太阳神"))
            enemy.受伤时(光照范围()*攻击效果());

        if(enemy!=null&&enemy.isAlive()&&天赋(Talent.能量迸发))
            enemy.受伤时(天赋点数(Talent.能量迸发,0.125f)*Dungeon.energy*攻击效果());

        if(enemy!=null&&enemy.isAlive()&&符文("无极剑道"))
            enemy.受伤时(0.2f*最大攻击()*攻击效果());

        if(enemy!=null&&enemy.isAlive()&&符文("砍伤"))
            enemy.受伤时(0.5f*最大防御()*攻击效果());

        if(enemy!=null&&enemy.isAlive()&&符文("临终轻语"))
            enemy.受伤时(enemy.已损失生命(0.09f)*攻击效果());
        ...
        if(enemy!=null&&enemy.isAlive()&&enemy.第x次防御==1&&符文("夺金")){
            enemy.受伤时(damage*0.4f*攻击效果());
            Dungeon.gold(Math.round(new Gold().random().数量()*攻击效果()));
        }
        if(enemy!=null&&enemy.isAlive()&&符文("均衡打击")){
            enemy.受伤时(damage*攻击效果());
        }
        if(enemy!=null&&enemy.isAlive()&&subClass(HeroSubClass.灵剪刺客)){
            enemy.受伤时(enemy.最大生命((0.001f*魔力())*攻击效果()*(1+天赋点数(Talent.六芒星针)+(第x次攻击%4==0&&职业精通()?
                                                                                              1:
                                                                                              0))));
        }
        if(enemy!=null&&enemy.isAlive()&&天赋(Talent.引针族射)){
            enemy.受伤时(damage*天赋点数(Talent.引针族射,0.1f)*攻击效果());
        }
        ...
        if(enemy!=null&&enemy.isAlive()&&连击==0){
            ...
            if(符文("连拨击锤")){
                if(在上边(enemy.pos)&&nobuff(连拨击锤上冷却.class)){
                    Buff.施加(this,连拨击锤上冷却.class,8);
                    damage*=0.5f;
                    连击+=5*攻击效果();
                }
                if(在下边(enemy.pos)&&nobuff(连拨击锤下冷却.class)){
                    Buff.施加(this,连拨击锤下冷却.class,8);
                    damage*=0.5f;
                    连击+=5*攻击效果();
                }
                if(在左边(enemy.pos)&&nobuff(连拨击锤左冷却.class)){
                    Buff.施加(this,连拨击锤左冷却.class,8);
                    damage*=0.5f;
                    连击+=5*攻击效果();
                }
                if(在右边(enemy.pos)&&nobuff(连拨击锤右冷却.class)){
                    Buff.施加(this,连拨击锤右冷却.class,8);
                    damage*=0.5f;
                    连击+=5*攻击效果();
                }
            }
            ...
            if(连击>0)
                无动画连击(enemy,damage*攻击效果(),0,1);
        }

        if(enemy!=null&&enemy.第x次防御==1&&符文("先攻")){
            damage*=1.1f;
            Dungeon.gold(Math.round(damage*攻击效果()));
        }
    ...
    private float 蛇皮走位(Char enemy,float damage){
            蛇皮走位=Math.min(100,蛇皮走位+6*攻击效果());

        if(蛇皮走位==100){
            SpellSprite.show(this, SpellSprite.HASTE, 1, 1, 0);
            蛇皮走位=0;
            ...
            damage+=英雄伤害(最小防御(),最大防御())*天赋点数(Talent.步兵冲锋,0.25f);

            damage+=天赋点数(Talent.蓄意轰拳,0.5f)*力量();

            if (buff(Momentum.class)!=null&&buff(Momentum.class).freerunning()) {
                damage*=1+buff(Momentum.class).freerunTurns;
            }
            ...
            if(enemy!=null){
                回血(移速());
//                Buff.施加(this, Swiftthistle.TimeBubble.class).reset(1);
            }
            if(enemy!=null&&符文("盈能攻击！")&&enemy.isAlive())
                enemy.受伤时(enemy.生命(0.2f));

            if(enemy!=null&&符文("额外闪现")&&!belongings.hasItem(StoneOfBlink.class))
                new StoneOfBlink().数量(Math.round(攻击效果())).放背包();
    ...
    public float 增加闪避(Char target) {
        ...
        if(符文("唯物主义者")&&target!=null&&target instanceof Wraith) x*=1.75f;
        if(符文("会心闪避"))x=暴击判定(null,x);
        x *= 综合属性()*(1+天赋点数(Talent.下肢强化,0.1f));
        x*=闪避之戒.闪避(this);
    ...
    public int 最大闪避(Char enemy) {
        ...
        float evasion = 最大闪避 +(等级>=最大等级?等级+1:等级-1);
        if(heroClass(HeroClass.凌云)){
            evasion +=15;
        }
        evasion+=敏捷()/2f;
        ...
        evasionx+= 天赋点数(Talent.遮云见日, 0.25f);

        if(天赋(Talent.快速布阵)){
            evasionx+=(力量()-10)*属性增幅()/2f*(天赋点数(Talent.快速布阵,0.25f));
        }
        if(裸衣()){
...
//        }
        ...
        damage=物理抗性(enemy,damage);

        damage=防御特效(enemy,damage);
        ...
        if (belongings.armor1() != null) {
            damage= belongings.armor1().防御时(enemy,this,damage);
        }
        if (belongings.armor2() != null) {
            damage= belongings.armor2().防御时(enemy,this,damage);
        }
    ...
    private float 防御特效(Char enemy,float damage){

        if(enemy!=null&&符文("攻守兼备"))
            攻击特效(enemy, damage*5);
        if(enemy!=null&&Dungeon.系统(系统设置.防御成长)){
            防御成长+=Dungeon.depth/100f;
        }
        if(enemy!=null&&天赋(Talent.皇室传承)){
            Dungeon.gold(new Gold().random(天赋点数(Talent.皇室传承,0.15f)).数量());
        }
        ...
        if (subClass == HeroSubClass.苦行者)
        Buff.施加(this, Momentum.class).gainStack(0.75f);
        if(enemy!=null&&符文("吸元秘术")){
            if(hasbuff(吸元秘术冷却.class)){
                buff(吸元秘术冷却.class).spendConstant((enemy.老鬼()||enemy.小老鬼())?4.5f:3);
            }
        }
        if(enemy!=null&&符文("阴得没边")){
            Buff.施加( this, Invisibility.class, 3 );
            Buff.施加(this,必定命中.class);
            Buff.施加(this,必定暴击.class);
        }
        if(符文("装备死亡之舞"))
            if(damage*0.3f>0){
                if(enemy!=null){
                    ...
                    sprite.showStatus(CharSprite.警告橙,Messages.get(死舞.class,"deferred",damage*0.3f));
                }
                damage*=0.7f;
            }
        if(enemy!=null&&符文("混乱")&&算法.概率学(1/5f))
            ...
            for (int n : PathFinder.相邻){
                ...
                if(c!=null&&c.alignment == Alignment.ENEMY&&Dungeon.level.heroFOV[c.pos]){
                    ...
                    sprite.attack(enemy.pos,new Callback() {
                        ...
                        public void call() {
                            AttackIndicator.target(enemy);
                            if (attack(enemy,天赋点数(Talent.灵动尾击,0.4f),英雄伤害(武力之戒.heromin(),武力之戒.heromax()),1)) {
                                Sample.INSTANCE.play(Assets.Sounds.HIT);
                            }
                            Invisibility.notimedispel();
                            spendAndNext(0);
    ...
    private float 物理抗性(Char enemy,float damage){
        if(符文("我让你打我"))
            damage*=0.95f;
        if(enemy!=null&&视野范围(enemy))
            damage*=1-天赋点数(Talent.戒备,0.05f);
        if(天赋(Talent.欧皇庇护))
            damage*=1-Random.Float(0,天赋点数(Talent.欧皇庇护,0.2f));
        if(enemy!=null&&符文("外交豁免")){
            if(enemy.nobuff(外交豁免.class))
                damage*=0.25f;
        }
        ...
        if(符文("坚韧之躯"))
            damage*=1-0.2f*根据已损失生命();
        if(enemy!=null&&enemy.isAlive()&&符文("奉献"))
            enemy.受伤时(enemy.生命(0.3f));
        ...
        damage*=中国国旗.受伤();
        if(符文("防具的宠爱")&&belongings.armor1()!=null)
            damage*=1-(belongings.armor.tier()+belongings.armor.强化等级())/
                      (2f+(belongings.armor.tier()+belongings.armor.强化等级()));


        if(符文("防具的宠爱")&&belongings.armor2()!=null)
            damage*=1-(belongings.armor.tier()+belongings.armor2().强化等级())/
                      (2f+(belongings.armor.tier()+belongings.armor2().强化等级()));
        ...
        if(enemy!=null&&enemy.相邻(this))
            damage*=宝石肛塞.受伤();

        if(enemy!=null&&(老鬼()||小老鬼())&&符文("费米悖论")){
            damage*=0.8f;
        }

        if(enemy!=null&&(belongings.weapon instanceof 联合盾||belongings.secondWep instanceof 联合盾)&&算法.概率学(15)){
            damage=0;
        }
        return damage;
    }
    ...
    public void 受伤时(float dmg, Object 来源) {
        ...
        if(dmg>0){
            if(heroClass(HeroClass.巫女)&&算法.概率学(1/4f)){
                经验((int)Math.sqrt(dmg),getClass());
            }
            if(Dungeon.系统(系统设置.波罗神盾)&&dmg<=等级(0.25f)){
                Buff.施加(this,圣盾.class).增加(1);
            }
            if(符文("苦行僧"))
                Buff.延长(this,BlobImmunity.class,BlobImmunity.DURATION);
        }

        //英雄受伤时
        float preHP = 生命+ shielding();

        if (来源 instanceof Hunger) preHP -= shielding();
        super.受伤时(dmg,来源);


        float postHP = 生命+ shielding();

        if (来源 instanceof Hunger) postHP -= shielding();
    ...
    private float 受伤特效(float dmg,Object 来源){
        ...
        if(适应性头盔!=null&&符文("适应性头盔")){
            if(适应性头盔==来源) dmg*=0.8f;
        }
        适应性头盔=来源.getClass();

        if(符文("万世催化石")){
            Weapon.chargeWeapons(dmg*0.1f);
            Recharging.chargeWands(dmg*0.1f);
            ArtifactRecharge.chargeArtifacts(this,dmg*0.1f);
        }
        if(符文("血腥洞察")&&dmg>=最大生命(0.04f)){
            Buff.施加(this,必定暴击.class);
        }
        if(符文("淬炼血液")){
            生命成长+=dmg*0.03f;
        }
        Buff.施加(this,受伤累计.class).set(dmg);

        if(符文("倘若我说我绷住了呢")&&Random.Int(3)==0)
            dmg=0;
        ...
        for (Mob m : Dungeon.level.mobs.toArray(new Mob[0])) {
            ...
            if (fieldOfView[m.pos] && m.alignment == Alignment.ENEMY) {
                ...
                if ((!mindVisionEnemies.contains(m) && QuickSlotButton.autoAim(m) != -1)
                    ...
                    if (target == null) {
                        target = m;
                    } else if (距离(target)>距离(m)) {
                        target = m;
                    }
                    if (m instanceof Snake &&Dungeon.level.距离(m.pos,pos)<=4
...
//                AttackIndicator.updateState();
                ...
                if(符文("无妨我已经看到了未来"))
                    Buff.施加(this,Invulnerability.class,10);
    ...
    private boolean getCloser(final int target) {
        ...
        if(算法.isDebug()){
            ...
            if(c!=null){
                if(c!=this&&c.alignment==Alignment.ENEMY)
                    红=true;
            }
            if(Dungeon.level.在深渊(target))
                红=true;
...
//        }
        ...
        if (Dungeon.level.相邻(pos,target)) {
            ...
            if (path == null || path.isEmpty() || !Dungeon.level.相邻(pos,path.getFirst()))
                newPath = true;
    ...
    public void 移动处理(int gopot){
        ...
        if(符文("无休回复"))回血(0.005f*距离(gopot));

        if(符文("追雷赶电靴")){
            for (int n : PathFinder.相邻){
                Char c= Actor.findChar(pos+n);
                if(c!=null){
                    c.受伤时(移速()*敏捷()*距离(gopot));
                }
            }
        }
        if(符文("风雷翅:左风右雷随心意而动")){
    ...
    public boolean handle(int cell) {
        ...
        if (Dungeon.level.map[cell] == Terrain.ALCHEMY && cell != pos) {
            ...
            //TODO perhaps only trigger this if hero is already adjacent? reducing mistaps
                   ...
                   (Dungeon.区域()==3||Dungeon.level instanceof MiningLevel)&&
                   belongings.getItem(镐子.class)!=null&&
                   Dungeon.level.可挖矿(cell)) {

            curAction = new HeroAction.Mine(cell);

        } else if (!Dungeon.bossLevel()&&
                   belongings.getItem(钻石镐.class)!=null&&
                   Dungeon.level.可挖矿(cell)) {
            curAction = new HeroAction.Mine(cell);
...
//                        Dungeon.depth < 26 || //26+不能上楼
    ...
    public void 护甲(float x){
        if(x>0&&妖精粉尘.获得()>0&&算法.概率学(1/10f/3f)){
            Dungeon.energy(Math.round(x*妖精粉尘.获得()));
        }
        if(x>0)
            x*=治疗护盾();
    ...
    public void 回血(float x){
        ...
        if(x>0){
            ...
            if(天赋概率(Talent.贪欲燔祭,Math.round(x)))
                Generator.随机物品().放背包();

            Buff.施加(this,回血累计.class).set(x);
            if(符文("当心小蛋糕"))
	...
	public void 经验(int exp,Class source) {
        ...
        if (sprite!=null&&exp > 0) {
           sprite.showStatusWithIcon(CharSprite.增强绿,Integer.toString(exp),FloatingText.经验数值);
        }
        ...
        if (source != AscensionChallenge.class) {
            ...
            if (heroClass(HeroClass.MAGE)) {
                Recharging.chargeWands(exp/10f);
            }
        }
    ...
    public void 升级(boolean 未满级) {
        ...
        if(未满级){
            ...
            if(Dungeon.派对(派对设置.海克斯)){
                if(等级==6||等级==11||等级==16||等级==21) {//7/13/19
                    new 海克斯秘卷().放背包();
                }
            }
        ...
        if(符文("你已经是一名成熟的英雄了")){
            ...
            回满护甲();
            if(hasbuff(Hunger.class))
                buff(Hunger.class).吃饭(Hunger.STARVING);
        }
...
//        }
    ...
    public void 死亡时(Object 来源) {
        ...
        if(符文("591铂金60金15银3铜"))
            Dungeon.gold((591*100*100*100+60*100*100+15*100+3)/5);

        if(符文("海克斯获取:血祭"))new 海克斯秘卷(true).放背包();
        ...
        if(符文("这波不活了"))
            Buff.延长(this,Invulnerability.class,150);

        if(符文("不死意志"))治疗成长+=0.05f;
        if(符文("魔术基盘"))魔力+=5;
        ...
        if(来源!=null){
            ...
            Weapon 玉龙=belongings.getItem(玉龙.class);
            if(ankh!=null
               ||(玉龙!=null&&玉龙.等级()>0)
               ||符文("作弊我能复活")
               ||符文("我无限回档洞悉所有底牌")
               ||符文("我无限回档洞悉所有底牌")
               ||(符文("守护天使")&&nobuff(守护天使冷却.class))
               ||(符文("广告复活")&&nobuff(广告复活冷却.class))
            ){
                interrupt();
                Badges.解锁罗兰();
                ...
                if(玉龙!=null){
                    ...
                    回百分比血(0.2f);
                    Buff.施加(this,守护天使冷却.class,300/能量之戒.weaponChargeMultiplier(this));
                }else if(符文("我无限回档洞悉所有底牌")){
                        回满血();
                        ...
                        if(来源 instanceof Hero.Doom){
                            ((Hero.Doom)来源).onDeath();
                        }
    ...
    public void move(int step, boolean travelling) {
        ...
        if (!flying && travelling) {
            if (Dungeon.level.water[pos]) {
                ...
                if(Dungeon.区域()==5){
                    Sample.INSTANCE.play(Assets.Sounds.瓷砖, 0.8f, Random.Float( 1.16f, 1.25f ) );
    ...
    public void onAttackComplete() {
        ...
        if(attackTarget.isAlive()&&天赋(Talent.错失良机)){
            Buff.延长(this,Invisibility.class,天赋点数(Talent.错失良机,2));
        }
//        if (hit && heroClass(HeroClass.DUELIST) && wasEnemy) {
//            Buff.施加(this, 双剑.ComboStrikeTracker.class).addHit();
//        }
    ...
    public boolean 视野范围(Char c){
        return Dungeon.level.heroFOV[c.pos];
    }
    @Override
    public int 视野范围() {
        float x = super.视野范围();
    ...
    public boolean search(boolean intentional) {
        ...
        int left, right;
        int curr;
        float 搜索时间=(搜索范围()+1)*(搜索范围()+1)/2f;
        if(subClass(HeroSubClass.神偷无影)&&职业精通())搜索时间*=0.5f;
        搜索时间*=1-天赋点数(Talent.边搜边打,0.15f);
        if(搜索范围()<=1){
            搜索时间+=遗失符石.减少()*3;
        }
        ...
        for (int y = Math.max(0, c.y - distance); y <= Math.min(Dungeon.level.height() - 1, c.y + distance); y++) {
            ...
            for (curr = left + y * Dungeon.level.width(); curr <= right + y * Dungeon.level.width(); curr++) {
                if (intentional){
                    ...
                    if (Dungeon.level.可挖矿(curr)&& Dungeon.level.insideMap(curr)){
                        if(!Dungeon.bossLevel()&&(Dungeon.区域()==3||Dungeon.level instanceof MiningLevel)&&belongings.getItem(镐子.class)!=null&&Dungeon.level.可挖矿(curr)){
                            if(挖矿处理(curr,false))
                                搜索时间++;
                        }else if(!Dungeon.bossLevel()&&belongings.getItem(钻石镐.class)!=null&&Dungeon.level.可挖矿(curr)){
                            if(挖矿处理(curr,false))
                            搜索时间++;
                        }
                    }
    ...
    public float 治疗护盾(){
        ...
        x+=0.01f*力量();
        x+=治疗成长;
        x*=1+天赋点数(Talent.活着,0.25f/3f);

        if(符文("花岗岩护盾"))x+=GameMath.百分之内(0.1f,根据护甲(),0.9f)/3f;
        ...
        if(符文("英雄平衡性增益"))x+=0.2f;
        if(符文("吸血鬼"))x+=吸血()+全能吸血();
        if(符文("真:吸血鬼")&&(吸血()+全能吸血())>0.25f)x+=2.5f;
        if(符文("无限火力"))x-=0.5f;
        if(符文("急救用具"))x+=0.25f;
        if(符文("回归基本功"))x+=0.45f;
        if(符文("多克特"))x+=0.60f;
        ...
        x+=天赋点数(Talent.天命眷顾,0.3f)*暴击率();
        x+=天赋点数(Talent.死亡抗拒,0.25f)*根据已损失生命();
        if(符文("猩红诅咒"))
            x+=2*根据已损失生命();

        if(符文("会心治疗")&&暴击判定(null,1)>1) x*=1.45f;
	...
	public float 综合属性() {
        ...
        if(subClass(HeroSubClass.灾厄化身))x+=0.25f;
        if(符文("黄金忍者")&&(
                belongings.hasItem(火焰剑.class)||
                belongings.hasItem(地裂镰.class)||
                belongings.hasItem(寒冰镖.class)||
        ...
        if(belongings.hasItem(金铲铲.class))x+=0.2f;
        x+=天赋点数(Talent.女中豪杰,0.05f);
        if(hasbuff(暴雨层.class))x+=Buff.施加(this,暴雨层.class).count*0.05f;
        if(符文("禁忌物001")){
            int 禁忌物数量=0;
            for(Item i: belongings.backpack){
                if(i instanceof Trinket a)
                    禁忌物数量+=1+a.等级();
            }
            x+=禁忌物数量*0.2f;
        }
        ...
        x+=(GregorianCalendar.getInstance().get(Calendar.MONTH)+1)*
           GregorianCalendar.getInstance().get(Calendar.DAY_OF_MONTH)*0.01f;

        if(符文("腺上激素")&&生命<=最大生命(0.0075f))x+=3.5f;
        if(符文("升级升级:综合"))x+=0.2f*等级;
        if(符文("炒能力"))x+=Dungeon.gold/600f;
        x+=天赋点数(Talent.洗筋伐髓,0.075f)*根据已损失护甲();
        x+=天赋点数(Talent.八门遁甲,0.1f)*根据已损失生命();

        if(belongings.weapon(海神三叉戟.class)&&在水中()){
            x+=0.2f;
        }
        ...
        if(符文("黄金幻影忍者")){
            if(belongings.hasItem(火焰剑.class)) x+=1;
            if(belongings.hasItem(地裂镰.class)) x+=1;
            if(belongings.hasItem(寒冰镖.class)) x+=1;
            if(belongings.hasItem(闪电双截棍.class)) x+=1;
        }
        if(符文("子时已到")){
            if(Dungeon.地牢时间>=时间(2300)||Dungeon.地牢时间<=时间(100))x+=5;
        }
        if(Dungeon.地牢时间>=时间(1800)||Dungeon.地牢时间<=时间(600))x+=天赋点数(Talent.夜行专精,0.15f);
        if(种族天赋.equals("人类"))x+=0.05f;
        if(种族天赋.equals("鲛人")&&在水中())x+=0.15f;
        if(种族天赋.equals("精灵")&&在草丛())x+=0.15f;
        if(符文("行万里路"))x+=0.25f;
        if(符文("没有魔力的魔法帝"))x+=2.75f;
        ...
        if(符文("我是地牢大佬")){
            x+=Challenges.activeChallenges()*0.1f;
            x+=炼狱设置.使用()*0.2f;
            x+=赛季设置.使用()*0.3f;
            x-=解压设置.使用()*0.1f;
            x-=系统设置.使用()*0.2f;
        ...
        if(subClass(HeroSubClass.指环王))x-=0.2f;
        ...
        if(符文("5号化合剂"))x+=化合剂*0.01f;
        if(符文("暗黑破坏神"))x+=破坏物体*0.0045f;
        ...
        if(符文("升级缝合像素地牢"))x*=1.15f;
        if(符文("地牢原点")&&Dungeon.depth==13)x*=3;
        if(符文("神赐")&&hasbuff(Bless.class)) x*=3;
        if(Dungeon.神英)x*=2.5f;
        if(符文("综合公式"))x*=2;
        if(hasbuff(替身保护.class)) x*=1.1f;
    ...
    public float 吸血() {
        ...
        if(符文("亮出你的剑"))吸血+=0.05f*攻击范围get;
        if(subClass(HeroSubClass.神兽之灵)){
            吸血+=0.02f+天赋点数(Talent.坚铁甲胄,0.02f);
        }
        if(重生怪物.equals("吸血蝙蝠"))吸血+=0.4f;
        if(符文("生命渗透压"))
            吸血 += 0.0007f*最大生命;
        ...
        if(符文("修罗血场"))
            吸血 += 0.15f;
        if(符文("多兰剑")) 吸血+=0.03f;
        吸血 += 天赋点数(Talent.铁布回音, 0.02f);
        吸血 += 天赋点数(Talent.生命汲取, 0.02f);
        if(belongings.weapon2()!=null){
            吸血 +=belongings.weapon2().吸血();
        }
        if(belongings.weapon2()!=null){
            吸血 +=belongings.weapon2().吸血();
        }
    ...
    public float 全能吸血() {
        ...
        if(天赋(Talent.冰封领域))
            if(Dungeon.level!=null&&Dungeon.level.blobs!=null){
                Freezing freezing = (Freezing) Dungeon.level.blobs.get(Freezing.class);
                if (freezing != null && freezing.volume > 0)
                    全能吸血+=freezing.volume/(12500f-天赋点数(Talent.冰封领域,2500f));
            }


        if(符文("术士果汁盒"))全能吸血+=0.007f*魔力();
        if(符文("贪婪之握")){
            全能吸血+=0.01f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_1))全能吸血+=0.01f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_2))全能吸血+=0.01f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_3))全能吸血+=0.01f;
            if(Badges.local.contains(Badges.Badge.BOSS_SLAIN_4))全能吸血+=0.01f;
        }
        if(符文("升级升级:吸血")) 全能吸血+=0.007f*等级;
        if(符文("打野")) 全能吸血+=0.07f;
        if(符文("猩红之力")&&belongings.hasItem(MysteryMeat.class)) 全能吸血+=0.003f*belongings.getItem(MysteryMeat.class).数量();

        if(符文("虚空腐蚀")&&hasbuff(战斗状态.class)) 全能吸血+=0.07f;
        if (subClass(HeroSubClass.解咒真人)&&职业精通())
            全能吸血 += 0.05f;
        ...
        if(符文("不详契约")){
            全能吸血+= Math.min(0.2f,根据已损失生命()*0.2f);
        }
        征服 z=buff(征服.class);
        if(z!=null){
            if(职业精通())
                全能吸血+=(0.02f+天赋点数(Talent.征服之姿,0.01f)+(职业精通()?0.02f:0))/12f*z.层数;
            else if(z.满层())
                全能吸血+=0.02f+天赋点数(Talent.征服之姿,0.01f)+(职业精通()?0.02f:0);
        }
        if (heroClass(HeroClass.血鬼)) {
            全能吸血 += 0.03f;
        }
        ...
        if(符文("吸血习性"))全能吸血+=0.2f;
        全能吸血 += 天赋点数(Talent.血饮狂舞, 0.004f)*攻击范围();
        全能吸血 += 天赋点数(Talent.高级吸血, 0.0125f);
        全能吸血+=吸血成长;
    ...
    public float 穿甲() {
        ...
        穿甲+=破防之戒.破防(this);
        if(符文("升级狂妄皇冠")&&hasbuff(狂妄皇冠.class))穿甲+=20;
        if(符文("真穿甲流")) 穿甲+=最大护甲;
        if(符文("装备禁忌时机")) 穿甲+=20;
        if(符文("装备禁忌时机")&&nobuff(战斗状态.class)) 穿甲+=15;
        if(符文("穿甲的宠爱"))穿甲+=最大防御()*0.15f;
        if(符文("穿山甲"))穿甲+=最大防御()*0.45f;
        if(符文("残暴之力"))穿甲+=8;
        if(heroClass(HeroClass.兽灵))穿甲+=0.25f*最大护甲;
    ...
    public float 护甲穿透() {
        ...
        if (subClass==HeroSubClass.狙击手&&职业精通()){
            护甲穿透+= 0.35f;
        }
    ...
    public float 法术穿透() {
        ...
        if(subClass(HeroSubClass.大魔法师)&&职业精通()) 法术穿透+=0.3f;
        法术穿透+=法术成长;
        return 法术穿透;
    }
	//endregion
	...
	//region 其他函数
    public void 空间传送(Integer target) {
        if (target == null){
            return;
        }
        if (buff(WarpBeaconTracker.class)!=null){
            final WarpBeaconTracker tracker = buff(WarpBeaconTracker.class);

            GameScene.show( new WndOptions(
                    new Image(sprite),
                    Messages.titleCase(name()),
                    Messages.get(WarpBeaconTracker.class,"window_desc",tracker.depth),
                    Messages.get(WarpBeaconTracker.class, "window_tele"),
                    Messages.get(WarpBeaconTracker.class, "window_clear"),
                    Messages.get(WarpBeaconTracker.class, "window_cancel")){
                ...
                protected void onSelect(int index) {
                    if (index == 0){
                        ...
                        buff(WarpBeaconTracker.class).detach();
                    }
                }
            } );
            ...
            tracker.attachTo(this);

            sprite.operate(target);
            Sample.INSTANCE.play(Assets.Sounds.TELEPORT);
            Invisibility.dispel();
                ...
                new Callback() {
                    ...
                    public void call() {
                        ...
                        for (int cell : aoe.cells) {
                            ...
                            //### Deal damage ###
                            Char mob = Actor.findChar(cell);
                            int damage = Math.round(Hero.英雄伤害(15,25)
                                                    * effectMulti);
                            ...
                            if (mob != null && mob != hero){
                                ...
                                if (finalWandCls == WandOfLightning.class){
                                    ...
                                    if(mob.alignment == Char.Alignment.ALLY || mob.buff(Charm.class) != null){
                                        ...
                                        if (shielding > 0){
                                            healing -= shielding;
                                            Buff.施加(mob, 护盾.class).设置(shielding);
                                        } else {
                                            shielding = 0;
                                        }
                                        mob.回血(healing);
                                        ...
                                        if (!mob.属性表().contains(Char.Property.UNDEAD)) {
                                            ...
                                            damage = Math.round(Hero.英雄伤害(15,25)*effectMulti);
                                            mob.受伤时(damage, Reflection.newInstance(finalWandCls));
                        ...
                        charsHit = Math.min(4, charsHit);
                        if (charsHit > 0&&false){
                            float shielding = charsHit*2.5f;
                            Buff.施加(hero, 护盾.class).设置(shielding);
                        }
...
//                }
    ...
    public void 穿越攻击( Hero hero, Integer target ,float dmg) {
        if (target != null) {
            ...
            hero.sprite.jump(hero.pos, cell,0,0, new Callback() {
                ...
                public void call() {
                    ...
                    Char ch = Actor.findChar(target);
                    if (ch != null && ch.alignment != hero.alignment){
                        hero.attack(ch, dmg, 0, Char.INFINITE);
                        hero.spendAndNext(0);
                    }
    ...
    public String 职业精通提示(){
        return 职业精通提示(subClass,true,职业精通());
    }
    public static String 职业精通提示(HeroSubClass sc,boolean 提示,boolean 职业精通){
        String s="";
        if(职业精通){
            switch(sc){
                default:
                ...
                case 黑白双子:
                    ...
                    break;
                case 猫头鹰:
                    s+= "一直飞行，且对视野范围、感知范围内的敌人的攻击伤害+15%。";
                    break;
                case 巫咒王鼠:
                    s+= "击杀掉落生肉概率为必定。";
                    break;
    ...
    public void 施法(){
        ...
        if(符文("守护灵"))Buff.施加(this,守护灵次数.class).set(10);
        if(符文("万世催化石"))回血(20);
        if(符文("不详契约"))受伤时(生命(0.03f));

        if(subClass(HeroSubClass.祭司))
            Buff.延长(this,Bless.class,Bless.DURATION);
    }
}
