...
public class 土遁 extends 目标忍术 {

	public static 土遁 INSTANCE = new 土遁();
	...
	private void placeWall( int pos, int knockbackDIR){
		if (!Dungeon.level.solid[pos]) {
			...
			if (ch != null && ch.alignment == Char.Alignment.ENEMY){
				WandOfBlastWave.throwChar(ch,new Ballistica(pos, pos+knockbackDIR, Ballistica.PROJECTILE),1,false,false,土遁.INSTANCE);
	...
	public static class LightWall extends Blob {
		...
		public void use(BlobEmitter emitter) {
			super.use( emitter );
			emitter.pour( MagicMissile.WhiteParticle.土墙, 0.02f );
		}
