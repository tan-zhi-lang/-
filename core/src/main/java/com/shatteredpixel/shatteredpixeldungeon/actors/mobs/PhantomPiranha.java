...
public class PhantomPiranha extends Piranha {
	...
	public void 受伤时(float dmg, Object 来源) {
		...
		if (dmgSource == null || !Dungeon.level.相邻(pos,dmgSource.pos)){
		...
		if (isAlive() && !(来源 instanceof Corruption)) {
			if (dmgSource != null) {
				if (!Dungeon.level.相邻(pos,dmgSource.pos)) {
	...
	private boolean teleportAway(){
		...
		if (!outFOVCandidates.isEmpty()){
			if (Dungeon.level.heroFOV[pos]) GLog.白(Messages.get(this,"teleport_away"));
