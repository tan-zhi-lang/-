...
public class Blacksmith extends NPC {
					...
					GameScene.show(new WndQuest(Blacksmith.this, msg1Final) {
						...
						public void hide() {
							...
							if (pick.doPickUp( Dungeon.hero )) {
								GLog.白(Messages.capitalize(Messages.get(Dungeon.hero,"you_now_have",pick.name())));
							} else {
	...
	public static class Quest {
		...
		public static ArrayList<Room> spawn( ArrayList<Room> rooms ) {
			if (!spawned && Dungeon.相对层数() > 11 && Random.Int( 15 - Dungeon.相对层数() ) == 0) {
...
//				type = Random.IntRange(1, 2);
		...
		public static void complete(){
			...
			if (gold != null){
				favor += Math.min(4000,gold.数量()*100);
