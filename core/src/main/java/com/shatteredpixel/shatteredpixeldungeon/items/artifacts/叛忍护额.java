...
public class 叛忍护额 extends Artifact {
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals(AC_CAST)) {
			...
			if (!isEquipped(hero) && !hero.天赋(Talent.轻便护额)) GLog.白(Messages.get(Artifact.class,"need_to_equip"));
			else if (cursed)       GLog.白(Messages.get(this,"cursed"));
			else {

				GameScene.show(new Wnd忍术(this,hero,false));
	...
	private 忍术 quickSpell = null;

	public void setQuickSpell(忍术 spell){
		if (quickSpell == spell){
	...
	public class TomeRecharge extends ArtifactBuff implements ActionIndicator.Action {
		...
		public void doAction() {
			if (cursed){
				GLog.橙(Messages.get(叛忍护额.this,"cursed"));
				return;
			}

			if (!canCast(Dungeon.hero, quickSpell)){
				GLog.橙(Messages.get(叛忍护额.this,"no_spell"));
				return;
			}
