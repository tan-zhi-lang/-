...
import com.shatteredpixel.shatteredpixeldungeon.windows.Wnd法术;
...
public class 四叶草法典 extends Artifact {
	...
	public void setQuickSpell(法术 spell){
		if (quickSpell == spell){
	...
	public class TomeRecharge extends ArtifactBuff implements ActionIndicator.Action {
			...
			if (charge < chargeCap  && target.buff(MagicImmune.class) == null) {
				if (再生.regenOn()) {
					...
					if (!isEquipped(Dungeon.hero)){
						chargeToGain *= Dungeon.hero.天赋点数(Talent.轻便法典,0.25f);
					}
		...
		public Visual secondaryVisual() {
			Image ico= new ItemSprite(new 四叶草法典());
		...
		public int indicatorColor() {
			if (quickSpell==火球术.INSTANCE&&quickSpell.chargeUse(Dungeon.hero)==0){
				return 0x0063ff;
			} else {
				return 0x002157;
			}
		...
		public void doAction() {
			...
			if (QuickSlotButton.targetingSlot != -1 &&
					Dungeon.quickslot.getItem(QuickSlotButton.targetingSlot) == 四叶草法典.this) {
				targetingSpell = quickSpell;
				int cell = QuickSlotButton.autoAim(QuickSlotButton.lastTarget, 四叶草法典.this);
				...
				quickSpell.onCast(四叶草法典.this,Dungeon.hero);

				if (quickSpell.targetingFlags() != -1 && Dungeon.quickslot.contains(四叶草法典.this)){
					targetingSpell = quickSpell;
					QuickSlotButton.useTargeting(Dungeon.quickslot.getSlot(四叶草法典.this));
				}
			}
		}
	}
