...
public class 骷髅钥匙 extends Artifact {
	...
	public CellSelector.Listener targeter = new CellSelector.Listener(){
				...
				if (Dungeon.level.adjacent(target, curUser.pos)) {
					...
					if (Dungeon.level.map[target] == Terrain.LOCKED_DOOR){
						...
						curUser.sprite.operate(target, new Callback() {
							@Override
							public void call() {
								Buff.施加(curUser, KeyReplacementTracker.class).processIronLockOpened();
						...
						curUser.sprite.operate(target, new Callback() {
							@Override
							public void call() {
								Buff.施加(curUser, KeyReplacementTracker.class).processCrystalLockOpened();
						...
						curUser.sprite.operate(target, new Callback() {
							@Override
							public void call() {
								Buff.施加(curUser, KeyReplacementTracker.class).processGoldLockOpened();
		...
		public String prompt() {
			return Messages.get(骷髅钥匙.class, "prompt");
		}
	};
