...
public class 门神之戒 extends Ring {
	...
	public String statsInfo() {
		if (已鉴定()){
			String info = Messages.get(this, "stats",
									   kw2(1/8f*soloBuffedBonus()));
			if (isEquipped(Dungeon.hero) && soloBuffedBonus() != combinedBuffedBonus(Dungeon.hero)){
				info += "\n\n" + Messages.get(this, "combined_stats",
							  kw2(1/8f*combinedBuffedBonus(Dungeon.hero)));
			}
			return info;
		} else {
			return Messages.get(this, "stats", kw2(1/8f));
		}
	}

	public String upgradeStat1(int level){
		if (cursed && cursedKnown) level = Math.min(-1, level-6);
		return kw2(1/8f*(level));
	}

	
	public static float 累计(Char target){
		return 1/8f*getBuffedBonus( target, 财神爷.class);
	}
	...
	public static ArrayList<Item> tryForBonusDrop(Char target, float tries ){
		...
		int 概率00=5*x;
		int 概率11=10*x;
		int bonus = getBuffedBonus(target, 财神爷.class);
		...
		while ( triesToDrop.count <= 0 ){
			if (dropsToEquip.count <= 0){
				...
				for (财神爷 w : target.buffs(财神爷.class)){
	...
	public class 财神爷 extends RingBuff {
	}
