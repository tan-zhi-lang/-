...
public class ChaoticCenser extends Trinket {
	...
	public String statsDesc() {
		if (已鉴定()){
			return Messages.get(this, "stats_desc", kw2(averageTurnsUntilGas(等级())));
		} else {
			return Messages.get(this, "stats_desc", kw2(averageTurnsUntilGas(0)));
		}
	}
	...
	private static boolean produceGas( Char target ){
		...
		if (!candidateCells.isEmpty()) {
			...
			if (targetCell != null) {
				...
				GLog.橙(Messages.get(ChaoticCenser.class,"spew",Messages.titleCase(Messages.get(gasToSpawn,"name"))));
				if (target.sprite != null && target.sprite.parent != null) {
					target.sprite.parent.addToBack(new TargetedCell(targetCell));
				}
				return true;
			}
		}
