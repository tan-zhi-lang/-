...
public class ShardOfOblivion extends Trinket {
	...
	public static WndBag.ItemSelector identifySelector = new WndBag.ItemSelector() {
		...
		public void onSelect(Item item) {
			...
			if (ready){
				...
				GLog.绿(Messages.get(ShardOfOblivion.class,"identify"));
			} else {
				GLog.橙(Messages.get(ShardOfOblivion.class,"identify_not_yet"));
			}
		}
	};
	...
	public static float lootChanceMultiplier(int level){
		...
		if (Dungeon.hero.belongings.misc2() != null && !Dungeon.hero.belongings.misc2().已鉴定()){
			wornUnIDed++;
		}
		if (Dungeon.hero.belongings.misc3() != null && !Dungeon.hero.belongings.misc3().已鉴定()){
			wornUnIDed++;
		}
