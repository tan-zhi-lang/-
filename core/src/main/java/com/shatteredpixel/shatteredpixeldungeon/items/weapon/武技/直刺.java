...
public class 直刺 extends 武技{
	@Override
	public void 武技(Hero hero,Integer target){
		
		lungeAbility(hero, target, 1, 0, wep);
		super.武技(hero,target);
	}
	
	public static void lungeAbility(Hero hero, Integer target, float dmgMulti, int dmgBoost, Weapon wep){
		if (target == null){
			return;
		}
		...
		hero.sprite.jump(hero.pos, dest, 0, 0.1f, new Callback() {
			...
			public void call() {
				...
				if (enemy != null && hero.canAttack(enemy)) {
					...
					Weapon.Charger
							charger = Buff.施加(hero,Weapon.Charger.class);
