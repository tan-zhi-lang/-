...
public class 荣誉纹章 extends Item {
...
//		}
		...
		if (armor != null) {
			if (!armor.cursedKnown){
				...
				GameScene.show(new WndOptions(new ItemSprite(物品表.荣誉纹章),
						Messages.get(荣誉纹章.class,"choose_title"),
						Messages.get(荣誉纹章.class,"choose_desc",armor.glyph.name(),getGlyph().name()),
						armor.glyph.name(),
						getGlyph().name()){
					@Override
	...
	protected static WndBag.ItemSelector armorSelector = new WndBag.ItemSelector() {
		...
		public String textPrompt() {
			return  Messages.get(荣誉纹章.class,"prompt");
		}
