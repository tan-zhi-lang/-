...
public class VaultBookcaseTreasureRoom extends VaultTreasureRoom {
	...
	public void paint(Level level) {
		...
		if (entrance.x == left || entrance.x == right){
			int bookTop = (int)GameMath.之内(top+1, entrance.y-2, bottom-5);
			...
			int bookLeft = (int)GameMath.之内(left+1, entrance.x-2, right-5);
		...
		if (treasureItem.cursed){
			...
			if (((Weapon) treasureItem).hasCurseEnchant()){
				((Weapon) treasureItem).enchant(null);
			}
		}
		//not true ID
