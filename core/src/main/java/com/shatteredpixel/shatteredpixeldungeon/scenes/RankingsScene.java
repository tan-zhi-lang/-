...
public class RankingsScene extends PixelScene {
	...
	public void create() {
		...
		if (Rankings.INSTANCE.records.size() > 0) {
			...
			float rowHeight = GameMath.之内(ROW_HEIGHT_MIN,(uiCamera.height-26)/Rankings.INSTANCE.records.size(),ROW_HEIGHT_MAX);
...
//			add(new WndVictoryCongrats());
	...
	public static class Record extends Button {
		...
		public Record( int pos, boolean latest, Rankings.Record rec ) {
			...
			if (rec.win) {
				shield.copy( new ItemSprite(物品表.AMULET) );
				...
				if (rec.ascending){
					shield.copy( new ItemSprite(物品表.AMULET, null) );
		...
		protected void createChildren() {
			...
			shield = new Image(new ItemSprite( 物品表.TOMB, null ));
			add( shield );
