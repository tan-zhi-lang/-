...
//							     "生成概率1/2/3/4/5阶" +
										   ...
										   "力量超过武器力量，则攻击伤害+0~武器力量-力量->武器力量-力量\n" +
										   "装备两把武器时，代码判定真实等级最强的掉落遗产->DPS伤害最高的\n" +
										   "新增双手和双持武器类，双手武器，副武器如果有武器，则主武器仅副武器的效果\n" +
										   "装备和卸下时间1->攻击延迟\n" +
										   "5阶都是特别物品，且都伤害x1.25\n" +
										   "武器战技重做\n" +
										   "最小攻击为1" ,
...
//										   "武器最小攻击~最大攻击为阶+级~最大攻击=5x(阶+1)+级x(阶+1)->阶+级~最大攻击=5x(阶+1)+级x(阶+1)。\n" +
										   "武器投掷最小攻击~最大攻击为2x阶+阶x级~5x阶+阶x级->(阶+级)x1.5~(5x(阶+1)+级x(阶+1))x1.5。\n"
		));
	...
	public static void 重要(ArrayList<ChangeInfo> changeInfos ) {
		...
		changes.addButton( new ChangeButton("","英雄数值",
											...
											"武器在近距离最大命中不变，非近距离(抵近射击)根据距离最多-40%最大命中。\n"+
