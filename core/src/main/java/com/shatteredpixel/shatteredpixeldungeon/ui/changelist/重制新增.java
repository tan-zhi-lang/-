	...
	*/
	...
	private static void 新机制(ArrayList<ChangeInfo> changeInfos) {
		...
		"每个英雄有自己的主属性，最大攻击+主属性-10。\n"+
			"力量\n" +
			"1%治疗护盾，0.4%暴击率，2最大生命，因力量的空手伤害和满适配武器的额外伤害，满适配条件。",

				"敏捷\n"+
				"初始是10+0.4x升级等级(满级额外+0.4)，1最大护甲，影响命中、攻速、闪避、移速\n"+
		...
		changes.addButton( new ChangeButton(new Image(avatar(HeroClass.WARRIOR,tier(HeroClass.WARRIOR))),"杂项机制",
											...
											"新增护甲，初始是5(5+0.5x等级(满级额外+1))，相当于不会随时间损失的物理护盾(前提不在战斗状态)，并且拥有10%伤害减免，包含爆炸、骷髅死亡爆炸和跳楼受伤(不含流血)。非战斗状态的每150回合修理1护甲。\n" +
