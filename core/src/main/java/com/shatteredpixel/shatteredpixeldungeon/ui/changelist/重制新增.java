...
import com.shatteredpixel.shatteredpixeldungeon.items.weapon.无限宝石拳套;
...
import com.shatteredpixel.shatteredpixeldungeon.items.weapon.燧石;
	...
	*/
	...
	private static void 新机制(ArrayList<ChangeInfo> changeInfos) {
		...
		changes.addButton( new ChangeButton(new Image(avatar(HeroClass.WARRIOR,tier(HeroClass.WARRIOR))),"杂项机制",
											...
											"蛇皮走位，正常行动会叠加2层，攻击叠加6层，最多100层。满层时的攻击恢复移速的生命，并清空。\n"+
	...
	private static void 新武器(ArrayList<ChangeInfo> changeInfos) {
		...
		changes.addButton(new ChangeButton(new 燧石()));
		changes.addButton(new ChangeButton(new 蘑菇长矛()));
		...
		changes.addButton(new ChangeButton(new 臭虎爪()));
		...
		changes.addButton(new ChangeButton(new 奇袭终刃()));
		changes.addButton(new ChangeButton(new 无限宝石拳套()));
