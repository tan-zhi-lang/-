...
public class 上楼标 extends Tag {


		Visual primaryVis;
		Visual secondVis;

	public static 上楼标.Action action;
	public static 上楼标 instance;
		...
		public static void setAction(上楼 action){
			synchronized (上楼.class) {
				上楼标.action= action;
				refresh();
			}
		...
		public static void clearAction(上楼 action){
			synchronized (上楼.class) {
				if (action == null||上楼标.action==action) {
					上楼标.action= null;
				}
			}
		}
