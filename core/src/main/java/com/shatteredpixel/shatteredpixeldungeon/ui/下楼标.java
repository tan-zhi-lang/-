...
public class 下楼标 extends Tag {


		Visual primaryVis;
		Visual secondVis;

	public static 下楼标.Action action;
	public static 下楼标 instance;
		...
		public static void setAction(下楼 action){
			synchronized (下楼.class) {
				下楼标.action= action;
				refresh();
			}
		...
		public static void clearAction(下楼 action){
			synchronized (下楼.class) {
				if (action == null||下楼标.action==action) {
					下楼标.action= null;
				}
			}
		}
