...
import com.shatteredpixel.shatteredpixeldungeon.actors.hero.spells.死血之术;
	...
	public class SpellButton extends IconButton {
		...
		public SpellButton(巫术 spell, 灵月法杖 tome, boolean info){
			...
			if (!tome.canCast(Dungeon.hero, spell)){
				icon.alpha( 0.3f );
			} else if (spell == 死血之术.INSTANCE && spell.chargeUse(Dungeon.hero) == 0){
				icon.brightness(3);
			}
		...
		protected void onPointerDown() {
			super.onPointerDown();
			if (spell == 死血之术.INSTANCE && spell.chargeUse(Dungeon.hero) == 0){
				icon.brightness(4);
			}
		}
