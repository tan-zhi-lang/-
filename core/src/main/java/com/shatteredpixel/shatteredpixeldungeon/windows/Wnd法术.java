...
public class Wnd法术 extends Window {

	protected static final int WIDTH    = 120;

	public static int BTN_SIZE = 20;

	public Wnd法术(四叶草法典 tome,Hero cleric,boolean info){
		...
		IconButton btnInfo = new IconButton(info ? new ItemSprite(tome) : Icons.INFO.get()){
			...
			protected void onClick() {
				GameScene.show(new Wnd法术(tome,cleric,!info));
				hide();
			}
		};
		...
		for (int i = 1; i <= Talent.MAX_TALENT_TIERS; i++) {
			...
			ArrayList<法术> spells = 法术.getSpellList(cleric, i);
	...
	public class SpellButton extends IconButton {
		...
		public SpellButton(法术 spell,四叶草法典 tome,boolean info){
			...
			if (!tome.canCast(Dungeon.hero, spell)){
				icon.alpha( 0.3f );
			} else if (spell==火球术.INSTANCE&&spell.chargeUse(Dungeon.hero)==0){
				icon.brightness(3);
			}
		...
		protected void onPointerDown() {
			super.onPointerDown();
			if (spell==火球术.INSTANCE&&spell.chargeUse(Dungeon.hero)==0){
				icon.brightness(4);
			}
		}
		...
		protected void onRightClick() {
			...
			RightClickMenu r = new RightClickMenu(new Image(icon),
					...
					Messages.get(Wnd法术.class,"cast"),
					Messages.get(Wnd法术.class,"info"),
					Messages.get(Wnd法术.class,"quick_cast")){
				@Override
